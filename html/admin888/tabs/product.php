...
else
{
	...
	else
	{
        ...
        //die('categorys '.$product->category.' linkrewrite '.$product->link_rewrite.' id '.$product   ->id.' defaultrewrite '.$default_rewrite);
...
if($category->id_category==14)	
 $smarty->display(_PS_THEME_DIR_.'experiencias.tpl');
else $smarty->display(_PS_THEME_DIR_.'product.tpl');
*/	
$smarty->assign('PRR',$category->id_category);	
