...
class AdminProducts extends AdminTab
{
	...
	public function __construct()
	{
		...
		//echo('categ '.$this->_category->id);
		/*
		if ($this->_category->id!=_IDCAT_EVENTOS_EMPRESA_ && $this->_category->id!= _IDCAT_RUTAS_A_MEDIDA_)
		{
		...
		$this->fieldsDisplay = array(
			...
			'image' => array('title' => $this->l('Photo'), 'align' => 'center', 'image' => 'p', 'width' => 45, 'orderby' => false, 'filter' => false, 'search' => false),
			//'name' => array('title' => $this->l('Name'), 'width' => 220, 'filter_key' => 'b!name'),
			'name' => array('title' => $this->l('Name'), 'width' => 220, 'filter_key' => 'b!titulo'), 
			'wholesale_price' => array('title' => $this->l('Base price'), 'width' => 70, 'price' => true, 'align' => 'right', 'filter_key' => 'a!price'),
			'price_final' => array('title' => $this->l('Final price'), 'width' => 70, 'price' => true, 'align' => 'right', 'havingFilter' => true, 'orderby' => false),
			'position' => array('title' => $this->l('Position'), 'width' => 40,'filter_key' => 'cp!position', 'align' => 'center', 'position' => 'position'),
			'active' => array('title' => $this->l('Displayed'), 'active' => 'status', 'align' => 'center', 'type' => 'bool', 'orderby' => false));
		//}	
		...
		/*if (intval($this->_category->id)!=_IDCAT_EVENTOS_EMPRESA_ && intval($this->_category->id)!= _IDCAT_RUTAS_A_MEDIDA_)
			...
			//mts 12082013
			$this->_select = 'cp.`position`, i.`id_image`, a.`price` AS price_final';
	...
	public function getList($id_lang, $orderBy = NULL, $orderWay = NULL, $start = 0, $limit = NULL)
	{
		...
		if ($this->_list)
		{
			...
			//mts 12082013
				...
				for ($i = 0; $i < $nb; $i++)
					$this->_list[$i]['price_tmp'] = Product::getPriceStatic($this->_list[$i]['id_product'], false, NULL, 2, NULL, false, false,0,false);
		}
	...
	public function submitAddproduct($token = NULL)
	{
		...
		/*if (Tools::getValue('id_category') ==_IDCAT_EVENTOS_EMPRESA_ || Tools::getValue('id_category') == _IDCAT_RUTAS_A_MEDIDA_) 
		{*/
			$rules['required'] = array('price','titulo','subtitulo','wholesale_price','reduction_price','reduction_percent');
			//if (Tools::getValue('id_category') ==_IDCAT_RUTAS_A_MEDIDA_ || Tools::getValue('id_category') == _IDCAT_RUTAS_A_MEDIDA_)
				$name_required = array('name_3'=>'Id Interno','price'=>'precio final','titulo'=>'titulo','subtitulo'=>'subtitulo');
			//else
				//$name_required = array('name_3'=>'Id Interno','price'=>'precio final','titulo'=>'titulo','subtitulo'=>'subtitulo','wholesale_price'=>'precio','reduction_price'=>'ahorro','reduction_percent'=>'descuento');
		...
		foreach ($rules['required'] AS $field)
			if (($value = Tools::getValue($field)) == false AND $value != '0')
			{
				...
				//mts 13082013
				//if (Tools::getValue('id_category') ==_IDCAT_EVENTOS_EMPRESA_ || Tools::getValue('id_category') == _IDCAT_RUTAS_A_MEDIDA_)
					$this->_errors[] = $this->l('the field').' <b>'.$name_required[$field].'</b> '.$this->l('is required');
				//else				
		...
		foreach ($rules['requiredLang'] AS $fieldLang)
			...
			{
				//die($fieldLang.'_<'.$defaultLanguage->id.' '.Tools::getValue($fieldLang.'_'.$defaultLanguage->id));
				//if (Tools::getValue('id_category') ==_IDCAT_EVENTOS_EMPRESA_ || Tools::getValue('id_category') == _IDCAT_RUTAS_A_MEDIDA_)
					$this->_errors[] = $this->l('the field').' <b>'.$name_required[$fieldLang.'_'.$defaultLanguage->id].'</b> '.$this->l('is required at least in').' '.$defaultLanguage->name;
				//else				
		...
		foreach ($languages AS $language)
			if ($value = Tools::getValue('description_short_'.$language['id_lang']))
				if (Tools::strlen(strip_tags($value)) > 2000)
					$this->_errors[] = $this->l('the field').' <b>'.call_user_func(array($className, 'displayFieldName'), 'description_short').' ('.$language['name'].')</b> '.$this->l('is too long').' : 2000 '.$this->l('chars max').' ('.$this->l('count now').' '.Tools::strlen(strip_tags($value)).')';
		...
		foreach ($rules['validate'] AS $field => $function)
		{
			...
			if ($value = Tools::getValue($field))
			{
				...
				/*if (Tools::getValue('id_category') ==_IDCAT_EVENTOS_EMPRESA_ || Tools::getValue('id_category') == _IDCAT_RUTAS_A_MEDIDA_)
				{*/
					$this->_errors[] = $this->l('the field').' <b>'.$name_required[$field].'</b> '.$this->l('is invalid');
				/*}
				else		
...
//var_dump($object);die('aki27	');							
	...
	public function displayForm($isMainTab = true)
	{
		...
		<form action="'.$currentIndex.'&token='.Tools::getValue('token').'" method="post" enctype="multipart/form-data" name="product" id="product">
			...
			<div class="tab-pane" id="tabPane1">';
				/* Tabs */
		/*if ($id_category!=_IDCAT_EVENTOS_EMPRESA_ && $id_category!= _IDCAT_RUTAS_A_MEDIDA_)		
			$this->displayFormInformations($obj, $currency);
		else*/
			$this->displayFormInformationsEventosEmpresa($obj, $currency);		
			...
			<table cellpadding="5" style="width:100%">
				...
				<tr>
					...
					<td>
						...
						<p style="padding:0px; margin:0px 0px 10px 0px;">'.$this->l('Will be applied on unit price for each product').'</p>
					</td>
				</tr>
				<tr>
	...
	function displayFormInformations($obj, $currency)
	{
		...
		echo '		</td>
					...
					echo '
					<tr>
						<td class="col-left">T&iacute;tulo</td>
						<td style="padding-bottom:5px;">
							<input length="1000" size="82" name="titulo" type="text" value="'.htmlentities($this->getFieldValue($obj, 'titulo'), ENT_COMPAT, 'UTF-8').'" >
						</td>
						</tr>';					
					echo '
					<tr>
						<td class="col-left">Subt&iacute;tulo</td>
						<td style="padding-bottom:5px;">
							<input length="1000"  size="82"  name="subtitulo" type="text" value="'.htmlentities($this->getFieldValue($obj, 'subtitulo'), ENT_COMPAT, 'UTF-8').'" >
						</td>
						</tr>';					
...
?>
	...
	<tr>
		<td colspan="2">
			<p><input type="checkbox" id="is_virtual_good" name="is_virtual_good" value="true" onchange="toggleVirtualProduct(this)" onclick="toggleVirtualProduct(this);" <?php if(($productDownload->id OR Tools::getValue('is_virtual_good')=='true') AND $productDownload->active) echo 'checked="checked"' ?> />
	...
	<?php if(!$productDownload->checkFile()): ?>
		...
		<?php endif; ?>
					...
					<p><input type="file" id="virtual_product_file" name="virtual_product_file" value="" class="" onchange="uploadFile()" maxlength="<?php echo $this->maxFileSize ?>" /></p>
					<?php endif; ?>
...
<?php 			echo '
					<tr>
						...
						<td style="padding-bottom:5px;">
							'.($currency->format == 1 ? $currency->sign.' ' : '').'<input size="11" maxlength="14" name="wholesale_price" type="text"  onblur="if (this.value==\'\') this.value = 0;" value="'.htmlentities($this->getFieldValue($obj, 'wholesale_price'), ENT_COMPAT, 'UTF-8').'" onchange="this.value = this.value.replace(/,/g, \'.\');" />'.($currency->format == 2 ? ' '.$currency->sign : '').'
							<span style="margin-left:10px">'.$this->l('The wholesale price at which you bought this product').'</span>
						</td>
					</tr>';
					echo '
					<tr>
 <td class="col-left">Precio Socio</td>
 <td style="padding-bottom:5px;">
	<input size="15" maxlength="15" name="price_socio" type="text" value="'.htmlentities($this->getFieldValue($obj, 'price_socio'), ENT_COMPAT, 'UTF-8').'" >
 </td>
</tr>	
					<tr>
						<td class="col-left">'.$this->l('Pre-tax retail price:').'</td>
						<td style="padding-bottom:5px;">
							'.($currency->format == 1 ? $currency->sign.' ' : '').'<input size="11" maxlength="14" id="priceTE" name="price" type="text" value="'.$this->getFieldValue($obj, 'price').'" onchange="this.value = this.value.replace(/,/g, \'.\');" onkeyup="calcPriceTI();" onblur="if (this.value==\'\') this.value = 0;"/>'.($currency->format == 2 ? ' '.$currency->sign : '').'<sup> *</sup>
				...
				echo '</td>
					...
					<tr>
						...
						<td style="padding-bottom:5px;">
							'.($currency->format == 1 ? ' '.$currency->sign : '').' <input size="11" maxlength="14" id="priceTI" type="text" value="" onchange="noComma(\'priceTI\');" onkeyup="calcPriceTE();" onblur="if (this.value==\'\') this.value = 0;"/>'.($currency->format == 2 ? ' '.$currency->sign : '').'
					...
					<tr>
						...
						<td style="padding-bottom:5px;">
							'.($currency->format == 1 ? $currency->sign.' ' : '').'<input size="11" maxlength="14" id="ecotax" name="ecotax" type="text" value="'.$this->getFieldValue($obj, 'ecotax').'" onkeyup="formatEcoTax();" onblur="if (this.value==\'\') this.value = 0;"/>'.($currency->format == 2 ? ' '.$currency->sign : '').'
							<span style="margin-left:10px">('.$this->l('already included in price tax incl.').')</span>
						</td>
					</tr>
					<tr>
	   ...
	   //MTS 10032012 Si el producto es una escapada mostramos un texto diferente
       if (strpos(strtoupper(substr($obj->name[3],0,8)),'ESCAPADA') === false)
       {
		echo '</td></tr></table>
						</div>
					</td></tr>
					<tr><td colspan="2" style="padding-bottom:5px;"><hr style="width:100%;" /></td></tr>
					<tr><td class="col-left">Destacados<br /><br />';
					/*
						<i>('.$this->l('appears in search results').')</i></td>
						*/
					echo('						</td><td style="padding-bottom:5px;" class="translatable">');
       ...
       else
       {
		echo '</td></tr></table>
					...
					<tr><td colspan="2" style="padding-bottom:5px;"><hr style="width:100%;" /></td></tr>
					<tr>
						<td class="col-left">'.'Destacamos'.'<br /><br /><i>'.'</i></td>
						<td style="padding-bottom:5px;" class="translatable">';       
       }
		foreach ($this->_languages as $language)
			echo '		<div class="lang_'.$language['id_lang'].'" style="display: '.($language['id_lang'] == $this->_defaultFormLanguage ? 'block' : 'none').';float: left;">
		...
		echo '		</td>
					...
					<tr>
						<td class="col-left">Condiciones</td>
						<td style="padding-bottom:5px;" class="translatable">';
		
			echo '		
						<textarea class="rte" cols="100" rows="20" id="realiza" name="realiza">'.htmlentities(stripslashes($this->getFieldValue($obj, 'realiza')), ENT_COMPAT, 'UTF-8').'</textarea>
						';
		echo '		</td></tr>';
	   ...
	   echo '		</td>
					...
					<tr>
						...
						<td class="col-left">Descripci&oacute;n</td>
						<td style="padding-bottom:5px;" class="translatable">';
		//						<td class="col-left">Como se realiza</td>
       } 
       else // FIN MTS.
       {
       	 echo '		</td>
					...
					<tr>
						<!--<td class="col-left">'.$this->l('Description:').'</td>-->
						<td class="col-left">Descripi&oacute;n</td>
						<td style="padding-bottom:5px;" class="translatable">';
       }
		foreach ($this->_languages as $language)
			echo '		<div class="lang_'.$language['id_lang'].'" style="display: '.($language['id_lang'] == $this->_defaultFormLanguage ? 'block' : 'none').';float: left;">
	   ...
	   //MTS 22012013 A�adimos campo para link de video Youtube.
        echo '
                   <tr>
                    <td class="col-left">Link Video</td>
                    <td style="padding-bottom:5px;">
                        <input size="82" maxlength="500" name="link_video" type="text" value="'.htmlentities($this->getFieldValue($obj, 'link_video'), ENT_COMPAT, 'UTF-8').'" >
                    </td>
                   </tr>';	   
        echo '
                   <tr>
                    <td class="col-left">Link Video2</td>
                    <td style="padding-bottom:5px;">
                        <input size="82" maxlength="500" name="link_video2" type="text" value="'.htmlentities($this->getFieldValue($obj, 'link_video2'), ENT_COMPAT, 'UTF-8').'" >
                    </td>
                   </tr>';	   
		 
        echo '
                   <tr>
                    <td class="col-left">Link Video Dreamcars</td>
                    <td style="padding-bottom:5px;">
                        <input size="82" maxlength="500" name="link_video_dreamcars" type="text" value="'.htmlentities($this->getFieldValue($obj, 'link_video_dreamcars'), ENT_COMPAT, 'UTF-8').'" >
                    </td>
                   </tr>';	   
        echo '
                   <tr>
                    <td class="col-left">Link Video2 Dreamcars</td>
                    <td style="padding-bottom:5px;">
                        <input size="82" maxlength="500" name="link_video2_dreamcars" type="text" value="'.htmlentities($this->getFieldValue($obj, 'link_video2_dreamcars'), ENT_COMPAT, 'UTF-8').'" >
                    </td>
                   </tr>';	   
        ...
        echo '
                   <tr>
                    <td class="col-left">Link Video Hcc</td>
                    <td style="padding-bottom:5px;">
                        <input size="82" maxlength="500" name="link_video_hcc" type="text" value="'.htmlentities($this->getFieldValue($obj, 'link_video_hcc'), ENT_COMPAT, 'UTF-8').'" >
                    </td>
                   </tr>';	   
        echo '
                   <tr>
                    <td class="col-left">Link Video2 Hcc</td>
                    <td style="padding-bottom:5px;">
                        <input size="82" maxlength="500" name="link_video2_hcc" type="text" value="'.htmlentities($this->getFieldValue($obj, 'link_video2_hcc'), ENT_COMPAT, 'UTF-8').'" >
                    </td>
                   </tr>';	   
       ...
       {		
		echo '<tr >
						<td colspan="2"><br>------ CAMPOS ESPECIALES PARA SERVICIOS -------------------------------------------</td>';
		echo '		</td>
					</tr>
			<tr > 
						<td class="col-left">Caracteriisticas servicio</td>
						<td style="padding-bottom:5px;" class="translatable">';
		
			echo '		
						<textarea class="rte" cols="100" rows="20" id="Caract" name="Caract">'.htmlentities(stripslashes($this->getFieldValue($obj, 'Caract')), ENT_COMPAT, 'UTF-8').'</textarea>
						';
		echo '		</td>
					</tr>
					</tr>
						<tr>
						<td class="col-left">Promociones</td>
						<td style="padding-bottom:5px;" class="translatable">';
		
			echo '		
						<textarea class="rte" cols="100" rows="20" id="Promos" name="Promos">'.htmlentities(stripslashes($this->getFieldValue($obj, 'Promos')), ENT_COMPAT, 'UTF-8').'</textarea>
						';
		echo '		</td>
					</tr>
					';
       } /* Fin comprobaci�n escapadas */
       echo '
					<tr>
	...
	function displayFormInformationsEventosEmpresa($obj, $currency)
	{
/***/
		parent::displayForm();
		global $currentIndex, $cookie;
...
/*					echo '
					<tr>
						<td class="col-left">T&iacute;tulo</td>
						<td style="padding-bottom:5px;">
							<textarea rows="3" class="" style="width:98.5%" name="titulo" type="text" value="'.htmlentities($this->getFieldValue($obj, 'titulo'), ENT_COMPAT, 'UTF-8').'" >'.htmlentities($this->getFieldValue($obj, 'titulo'), ENT_COMPAT, 'UTF-8').'</textarea>
						</td>
						</tr>';					
					echo '
					<tr>
						<td class="col-left">Subt&iacute;tulo</td>
						<td style="padding-bottom:5px;">
							<textarea rows="3" class="" style="width:98.5%"  name="subtitulo" type="text" value="'.htmlentities($this->getFieldValue($obj, 'subtitulo'), ENT_COMPAT, 'UTF-8').'" >'.htmlentities($this->getFieldValue($obj, 'subtitulo'), ENT_COMPAT, 'UTF-8').'</textarea>
						</td>
						</tr>';					
*/
					
					if (Tools::getValue('id_category')==39) //plantillas env�o emails.
					{
						echo '
						<tr>
							<td class="col-left">Asunto</td>
							<td style="padding-bottom:5px;">
								<textarea rows="3" class="" style="width:98.5%" name="titulo" type="text" value="'.$this->getFieldValue($obj, 'titulo').'" >'.$this->getFieldValue($obj, 'titulo').'</textarea>
							</td>
							</tr>';					
						echo '
						<tr style="display:none;">
							<td class="col-left">Subt&iacute;tulo</td>
							<td style="padding-bottom:5px;">
								<textarea rows="3" class="" style="width:98.5%"  name="subtitulo" type="text" value="." >'.$this->getFieldValue($obj, 'subtitulo').'</textarea>
							</td>
							</tr>';					
					}
					else 
					{
						...
						<tr>
							<td class="col-left">Subt&iacute;tulo</td>
							<td style="padding-bottom:5px;">
								<textarea rows="3" class="" style="width:98.5%"  name="subtitulo" type="text" value="'.$this->getFieldValue($obj, 'subtitulo').'" >'.$this->getFieldValue($obj, 'subtitulo').'</textarea>
							</td>
							</tr>';					
					}

			echo '	<tr style="display:none;">
						<td class="col-left">Id Interno</td>
						<td style="padding-bottom:5px;" >';
		foreach ($this->_languages as $language)
			echo '		<div class="lang_'.$language['id_lang'].'" style="display: '.($language['id_lang'] == $this->_defaultFormLanguage ? 'block' : 'none').'; float: left;">
								<input style="width:608px;" size="55" type="text" id="name_'.$language['id_lang'].'" name="name_'.$language['id_lang'].'"
								value="'.substr($this->getFieldValue($obj, 'titulo'),0,40).'..."'.((!$obj->id) ? ' onkeyup="copy2friendlyURL();"' : '').' onkeyup="updateCurrentText();" onchange="updateCurrentText();" />
							</div>';
		echo '		</td>
					</tr> ';
		echo '<tr>
...
/*
		...
		if (Tools::getValue('id_category')!=39) //plantillas env�o emails.
		{
			echo '	 <tr><td colspan="2" style="padding-bottom:5px;"><hr style="width:100%;" /></td></tr>
						<tr>
							<td class="col-left">'.'Destacamos'.'<br /><br /><i>'.'</i></td>
							<td style="padding-bottom:5px;" class="translatable">';       
			...
			foreach ($this->_languages as $language)
				echo '		<div class="lang_'.$language['id_lang'].'" style="display: '.($language['id_lang'] == $this->_defaultFormLanguage ? 'block' : 'none').';float: left;">
									<textarea class="rte"  style="width:600px;height:500px;" wrap="hard" id="description_short_'.$language['id_lang'].'" name="description_short_'.$language['id_lang'].'">'.htmlentities(stripslashes($this->getFieldValue($obj, 'description_short', $language['id_lang'])), ENT_COMPAT, 'UTF-8').'</textarea>
								</div>';
			...
			echo '		</td>
				...
				echo '		
							<textarea class="rte"  style="width:600px;height:500px;" wrap="hard" id="realiza" name="realiza">'.htmlentities(stripslashes($this->getFieldValue($obj, 'realiza')), ENT_COMPAT, 'UTF-8').'</textarea>
							';
			echo '		</td></tr>';
			...
			foreach ($this->_languages as $language)
				echo '		<div class="lang_'.$language['id_lang'].'" style="display: '.($language['id_lang'] == $this->_defaultFormLanguage ? 'block' : 'none').';float: left;">
									<textarea class="rte"  style="width:600px;height:500px;" wrap="hard" id="description_'.$language['id_lang'].'" name="description_'.$language['id_lang'].'">'.htmlentities(stripslashes($this->getFieldValue($obj, 'description', $language['id_lang'])), ENT_COMPAT, 'UTF-8').'</textarea>
								</div>';		
		...
		if (Tools::getValue('id_category')!=39) //plantillas env�o emails.
		{
			...
			echo '		<tr >
							<td class="col-left">Caracter&iacute;sticas</td>
							<td style="padding-bottom:5px;" class="translatable">';
...
/*** fin textareas ***/						
	...
	//MTS 22012013 A�adimos campo para link de video Youtube.
		...
		echo '	 <tr><td colspan="2" style="padding-bottom:5px;"><hr style="width:100%;" /></td></tr>';
				...
				$precio = $this->getFieldValue($obj, 'wholesale_price');
				$precio = ($precio == number_format($precio,0))?number_format($precio,0):number_format($precio,2);
				$moneda = ($currency->format == 2 ? ' '.$currency->sign : '');
				if (Tools::getValue('id_category') ==_IDCAT_EVENTOS_EMPRESA_)
				echo '
						<tr>
							<td class="col-left">Precio</td>
							<td style="padding-bottom:5px;">
								'.($currency->format == 1 ? $currency->sign.' ' : '').'<input size="11" maxlength="14" name="wholesale_price" type="text"  onblur="if (this.value==\'\') this.value = 0;" value="'.htmlentities($precio, ENT_COMPAT, 'UTF-8').'" onchange="this.value = this.value.replace(/,/g, \'.\');" />'.$moneda.'
							</td>
						</tr>';
	 
				$descuento = $this->getFieldValue($obj, 'reduction_percent');
				$descuento = ($descuento == number_format($descuento,0))?number_format($descuento,0):number_format($descuento,2);
				
				$ahorro = $this->getFieldValue($obj, 'reduction_price');
				$ahorro = ($ahorro == number_format($ahorro,0))?number_format($ahorro,0):number_format($ahorro,2);
				
				$precio_final = $this->getFieldValue($obj, 'price');
				$precio_final = number_format($precio_final,2,'.','');
				//quitamos el formato : 
				//	$precio_final = ($precio_final == number_format($precio_final,0))?number_format($precio_final,0):number_format($precio_final,2);
				...
				echo '
						<tr>
							<td class="col-left">Descuento</td>
							<td style="padding-bottom:5px;">
								<input size="10" maxlength="14" type="text" name="reduction_percent" id="reduction_percent" value="'.$descuento.'" onblur="if (this.value==\'\') this.value = 0;" onkeyup="javascript:this.value = this.value.replace(/,/g, \'.\'); var key = window.event ? window.event.keyCode : event.which; if (key != 9) reductionPercent();" /> %     
							</td>
						</tr>
						<tr>
							<td class="col-left">Ahorro</td>
							<td style="padding-bottom:5px;">
								<input size="10" maxlength="14" type="text" name="reduction_price" id="reduction_price" value="'.$ahorro.'" onblur="if (this.value==\'\') this.value = 0;" onkeyup="javascript:this.value = this.value.replace(/,/g, \'.\'); var key = window.event ? window.event.keyCode : event.which; if (key != 9) reductionPercent();" /> %
							</td>
						</tr>
						';
				  echo '
						<tr>
							<td class="col-left"><b>Precio Final</b></td>
							<td style="padding-bottom:5px;">
								<input size="11" maxlength="14" id="price" name="price" type="text"  onblur="if (this.value==\'\') this.value = 0;" value="'.htmlentities($precio_final, ENT_COMPAT, 'UTF-8').'"  />
								'.$moneda.'
							</td>
						</tr>';
		} //fin comprobaci�n si plantilla.
	
			//on change de id="price" 
			//onchange="this.value = this.value.replace(/,/g, \'.\');"
/** Fin Precios **/
 
/** Campos obligatorios que no utilizaremos pero que nos exije la definici�n de la clase producto **/
    
	echo '<tr style="display:none;">';
	echo '<td class="col-left"><b></b></td>';
	
	echo '<td>';
	echo '<input type="hidden" id="link_rewrite_3" name="link_rewrite_3" value="test">';
	/*
	if (Tools::getValue('id_category', 1)== _IDCAT_EVENTOS_EMPRESA_)
		echo '<input type="hidden" id="id_category_default" name="id_category_default" value="'._IDCAT_EVENTOS_EMPRESA_.'">';
	else if (Tools::getValue('id_category', 1)== _IDCAT_RUTAS_A_MEDIDA_)
		echo '<input type="hidden" id="id_category_default" name="id_category_default" value="'._IDCAT_RUTAS_A_MEDIDA_.'">';
	*/
	echo '<input type="hidden" id="id_category_default" name="id_category_default" value="'.Tools::getValue('id_category', 1).'">';

	echo '<input type="hidden" id="id_tax" name="id_tax" value="0">';
	/*
	if (Tools::getValue('id_category', 1)== _IDCAT_EVENTOS_EMPRESA_)
		echo '<input type="checkbox" name="categoryBox[]" id="categoryBox13" value="'._IDCAT_EVENTOS_EMPRESA_.'" checked="checked">';
	else if (Tools::getValue('id_category', 1)== _IDCAT_RUTAS_A_MEDIDA_)
		echo '<input type="checkbox" name="categoryBox[]" id="categoryBox32" value="'._IDCAT_RUTAS_A_MEDIDA_.'" checked="checked">';
	echo '</td></tr>';
	*/
	echo '<input type="checkbox" name="categoryBox[]" id="categoryBox13" value="'.Tools::getValue('id_category', 1).'" checked="checked">';
/** Fin Campos oblig. **/
...
/**/		
	...
	function displayFormImages($obj, $token = NULL)
	{
		...
		<div class="tab-page" id="step2">
			...
			foreach ($images AS $k => $image)
			{
				...
				$image_newsletter='../img/p/'.$obj->id.'-'.$image['id_image'].'.jpg';
				...
				echo '
					...
					<td class="center">
					 ...
					 if($obj->id=='25')
					  {	
					    echo '<a class="newsletter" href="#" >Envia newsletter: <img src="../img/admin/edit.gif"  /></a>
					   ';
					  }
					echo '
					</td>
	...
	<script>
		$(document).ready(function(){
			$(".newsletter").colorbox({width:"60%", inline:true, href:"#news"});
		});
	</script>
			  
			  
	<div style='display:none'>
		<div id='news' style='text-align:left;padding:10px; background:#fff;'>
		
			  <iframe frameborder="0" style="border:none" src="tabs/iframenews.php?image_newsletter=<?php echo $image_newsletter ?>&base=<?php echo __PS_BASE_URI__ ?>&root=<?php echo _PS_ROOT_DIR_ ?>" width="100%" height="530">
