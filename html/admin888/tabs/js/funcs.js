function canvia_tipus(t) {
  ...
  if ($('#ciudad_rutas_turisticas').attr('class')!='boton_menu menu_activo')
	  {
	  	var tipus_arr = new Array('ferrari','_bferrari_','lamborghini','_blamborghini_','_buggy_','_porsche_','_bporsche_','_corvette_','_bcorvette_','formula');                                                                    
	  }
  else
  	 {
  ...
  {
	if (tipus_arr[i]=='ferrari' ||tipus_arr[i]=='_bferrari_' ||tipus_arr[i]=='_bporsche_'  ||tipus_arr[i]=='_porsche_'||tipus_arr[i]=='_bcorvette_' ||tipus_arr[i]=='_corvette_'||tipus_arr[i]=='formula') clase='boton_mediano';
  ...
  switch(t){
	  ...
	  break;
	  case 'formula':
	     id_('tipus_img').innerHTML='<span style="display:inline-block;float:left;position:relative;top:22px;left:10px;color:#ffffff;font-weight:bold;"></span><img style="padding:4px;border:1px solid #000" width="100px" alt="" src="../../img/c/formula.jpg">';
	  break;
	  // BUGGY
	  case '_buggy_':
...
/*
  ...
  */
  ...
  var ciudades = new Array('barcelona','madrid','valencia','andalucia','cantabria','circuitozaragoza','circuitovendrell','circuitomoradebre','circuitosegovia','circuitoandalucia','circuitovalencia','vitoria');             
  ...
  //Modificamos el tipo de vehículo por defecto y mostramos los vehículos correspondientes,
  //según la ciudad seleccionada.
  if (aux_t.substring(0,7)=='circuit')
  {
	  var aux_t_arr = aux_t.split('circuito');
	  
	  if (aux_t_arr[1]=='moradebre')
	  {
		  id_('ciudad_info_p').innerHTML ='circuito Mora d\'Ebre';
	  }
	  else
	  {
		  id_('ciudad_info_p').innerHTML ='circuito '+aux_t_arr[1];  
	  }
  ...
  else
  {
	  ...
	  $('#grupo_tipo_eventos').show();
	  $('a#ferrari').show();
	  $('a#_bferrari_').show();
	  $('a#lamborghini').show();
	  $('a#_blamborghini_').show();
	  $('a#_porsche_').show();
	  $('a#_bporsche_').show();
	  $('a#_corvette_').show();
	  $('a#_bcorvette_').show();
...
function ocultar_form_busqueda_baja_clientes(){
	id_('buscar_baja_clientes').style.display='none';
	id_('form_busqueda_baja_clientes').style.display='none';
}

function mostrar_form_busqueda_baja_clientes(){
	
	id_('buscar_baja_clientes').style.display='block';
	id_('form_busqueda_baja_clientes').style.display='block';
}
...
function print_organizadores_20km7km_todos(listado)
{

	if ($('#header_graella').css('display')=='none')
		{
		...
		{
		//get_graella_organizadores_20km7km_todos();
		abrir_formulario_organizadores_20km7km_todos();
		}
//document.getElementById('form_excel').submit();	  
}
/** fin seccion ofertas **/
