<?php  
 ...
 ?>
	 <script type="text/javascript" src="<?php echo $base_tabs ?>/js/funcs_sel.js"></script>
	 <script>
...
<div id="reubicar_evento" style="display:none;float:left;width:100%;" >
	...
	<FORM ACTION="javascript:;" onsubmit="envia_formulari()" METHOD="POST" id="form_reubicar" name="form_reubicar">
		...
		<div id="seleccion_dia">
			<span style="color:#006699;margin-right:524px;font-weight:bold;font-size:14px;">Seleccione ciudad y la fecha d&oacute;nde desea reubicar el evento</span>  
		  <fieldset>
            ...
            <fieldset>      
              ...
              <a id="ciudad_circuitovendrell" name="ciudad_c" class="boton_menu menu_activo" href="javascript:canvia_ciudad_sel('circuitovendrell')">Circ. El Vendrell</a> 
              <a id="ciudad_circuitomoradebre" name="ciudad_c" class="boton_menu" href="javascript:canvia_ciudad_sel('circuitomoradebre')">Mora d'Ebre</a> 
              <a id="ciudad_circuitoandalucia" name="ciudad_c" class="boton_menu" href="javascript:canvia_ciudad_sel('circuitoandalucia')">Circ. Andalucía</a> 
              <a id="ciudad_circuitosegovia" name="ciudad_c" class="boton_menu" href="javascript:canvia_ciudad_sel('circuitosegovia')">Circ. Segovia</a> 
              <a id="ciudad_circuitozaragoza" name="ciudad_c" class="boton_menu" href="javascript:canvia_ciudad_sel('circuitozaragoza')">Circ. Zaragoza</a> 
              <a id="ciudad_circuitovalencia" name="ciudad_c" class="boton_menu" href="javascript:canvia_ciudad_sel('circuitovalencia')">Circ. Valencia</a> 
            </fieldset> 
    		  ...
    		  <div style="float:left;width:33%;margin-top:30px;font-weight:bold;font-size:20px"><input type="button" id="boton_vehiculos" value="Cambiar Deportivo" onclick="actualizar_deportivo_sel();"></div>
    		  </fieldset>
		<?php 
		...
		<!-- IMPRIMIR -->
		<form target="_blank" id="form_g" action="<?php echo $base_tabs.'/print.php' ?>" method="POST" >
		...
		<!-- PDF -->
		<form target="_blank" id="form_pdf" action="<?php echo $base_tabs.'/download_pdf.php' ?>" method="POST" >
		...
		function actualizar_deportivo_sel()
		{
			...
			*/
			...
			$.colorbox({width:'90%',height:'550px', inline:true, href:'#form_reubicar',open:true});		
		}
		...
		function volver_calendario()
		{
		   ...
		   $.colorbox({width:"90%",height:"450px", inline:true, href:"#form_reubicar",open:true});		   
		}
		...
		function get_graella_sel(dia) {
		  ...
		  if (ciudad_aux_sel=='Barcelona') ciudad_aux_sel='';
		...
		*/
		...
		{ 
		  ...
		  var idevinserted='';
		  var result_modif = new Array();
		  
		  //id_('alta').style.display="block"
		  
		  if (confirm('El evento '+tipo_origen.toUpperCase()+' de '+((ciudad_origen=='')?'Barcelona':ciudad_origen)+' va a ser reubicado del dia "'+fecha_origen+' a las '+hora_orig+'h." al evento '+tipo.toUpperCase()+' de '+((ciudad=='')?'Barcelona':ciudad)+' del dia "'+fecha+' a las '+hora_+'h."\n\n Esta seguro de que desea continuar?'))
		  {
			...
			{   
				result_modif=ru.split('#');                         
				
				alert(result_modif[1]);                                            
				
				if (result_modif[0]!='')  
				{
				    idev=parseInt(result_modif[0]);                     
				}
				
				emailr(idev,hora,ciudad);             
		...
		function emailr(id,t,ciudad) 
		{
		  if (confirm("Estas seguro que deseas enviar el email") ) 
		  {		
				
				r = ajax.load('<?php echo $base_scripts ?>ajax.php?email_=' + id + '&ciudad='+ciudad+'&id_event=' + t);         
				alert(r);
		  }
		}
 ...
 */
		...
		//alert(id_('ciudad_origen').value);
		ciudad_origen = id_('ciudad_origen').value;
		hora_origen = id_('hora_origen').value;
		//alert(id_('tipo_origen').value+' '+ciudad_origen);
		
		if (id_('tipo_origen').value=='formula')
		{
			id_('tipus_img').innerHTML='<img style="padding:4px;border:1px solid #000" width="100px" alt="" src="../../img/c/formula.jpg">';
		}
		...
		//$ruta_disponibles=dirname(__FILE__).'/../disponibles.php';

		//include_once ($ruta_disponibles);
		?>


		<?php 
		$ruta_modals=dirname(__FILE__).'/../modals.php';
		include_once ($ruta_modals);
		?>
		   </body>
		 </html>
