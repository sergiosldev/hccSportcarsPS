...
function getTiposServicio()
{
	...
	$query = Db::getInstanceOf()->ExecuteS($sql);
    $servicios = array();
...
//die($sql);
    ...
    foreach ($query as $row)
    {
     //****** 
        ...
        $Oferta = new Oferta_(intval($row["id_oferta"]),$row["id_desc"], $row['name'], $row['subtitle'], $row['description_short'], $row["realiza"], $row['description'],$price, $row['reduction_percent'],$reduction_price,$final_price,$row['date_add'],$row['date_upd'],$row['active'],$row['observaciones'],$row['cliente_especial'],$row['descripcion_cupones'],$row['multiple_cantidad'],$row['orden'],$row['link_video'],$row['link_video2'],$row['fecha_alta'],$row['id_tipo_servicio'],$row['hcc'],$row['GT'],$row['periodo_automatico']);
		$Oferta->linkVideoGT = $row["link_video_GT"];	
		$Oferta->linkVideo2GT = $row["link_video2_GT"];	  
...
function DeleteOpcionOferta($inOpcion=null)
{
   ...
   if ($inOpcion!=0)
   {
	   ...
	   $result= Db::getInstanceOf()->Execute($sql);
...
function CompactarOrdenOfertas($entidad='campanyas')
{
    ...
    if ($entidad == 'ofertas')
    {
        ...
        $result= Db::getInstanceOf()->ExecuteS($sql);
        ...
        foreach ($result as $r)
        {
            ...
            $result2= Db::getInstanceOf()->Execute($sql);
...
function GetOfertasHistorico($inId=null,$inIdOferta=null,$orden=null,$clienteEspecial=null,$filtroCampanyas=null,$empresa=null,$filtroTipoServicio=null) 
{
    ...
    foreach ($query as $row)
    {
     //******
        ...
        $Oferta = new OfertaHistorico(intval($row["id_oferta_hist"]),intval($row["id_oferta"]),$row["id_desc"], $row['name'], $row['subtitle'], $row['description_short'], $row["realiza"], $row['description'],$price, $row['reduction_percent'],$reduction_price,$final_price,$row['date_add'],$row['date_upd'],$row['active'],$row['observaciones'],$row['cliente_especial'],null,$row['orden'],$row['caducada'],$row['descripcion_cupones'],$row['multiple_cantidad'],$row['link_video'],$row['link_video2'],$row['fecha_alta'],$row['id_tipo_servicio'],$row['hcc'],$row['GT'],$row['periodo_automatico']);
        $Oferta->idDescOpcion = $row["id_desc_opcion"];
		$Oferta->linkVideoGT = $row["link_video_GT"];	
		$Oferta->linkVideo2GT = $row["link_video2_GT"];	  
		$Oferta->ubicaciones = $row["ubicaciones"];
...
{          
    ...
    $sql = " insert into ps_oferta (id_oferta,orden,id_desc,id_desc_opcion,id_category_default,price,reduction_percent,active,reduction_price,final_price,realiza,descripcion_cupones,multiple_cantidad,link_video,link_video2,id_tipo_servicio,link_video_GT,link_video2_GT,ubicaciones) ";                                                                
 
    $sql = $sql. " values (".$NewId.",".$NewOrden.",'".$inIdDesc."','".$inIdDescOpcion."',2,'".$inPrecioValor."','".$inDescuento."',0,'".$inAhorro."','".$inPrecioFinal."','".$inCondiciones."','".$inDescripcionCupones."','".$inMultipleCantidad."','".$inLinkVideo."','".$inLinkVideo2."','".$inIdTipoServicio."','".$inLinkVideoGT."','".$inLinkVideoGT2."','".$inUbicaciones."')";                                                                              

    $result=Db::getInstanceOf()->Execute($sql);

	foreach ($inIdProvincias as $idprov)
	{
		if ($idprov) $idprov=intval($dprov);
		else $idprov="NULL";
		$sql2= " insert into  ps_ofertas_provincias (id_oferta,id_provincia) values (".$NewId.",".$idprov.")";	
		$result2=Db::getInstanceOf()->Execute($sql2);
	}
...
function InsertarImagenesOfertaADuplicado($idoferta,$idoferta_dest)
{
    ...
    $results= Db::getInstanceOf()->ExecuteS($sql);
    ...
    foreach($results as $r)
    {
        ...
        $result=Db::getInstanceOf()->Execute($insert);
...
function InsertOfertaHistorico($inIdOferta,$inIdDesc,$inTitulo, $inSubtitulo, $inDestacados, $inCondiciones, $inDescripcion, $inPrecioValor,$inDescuento,$inAhorro,$inPrecioFinal,$inFechaInicio,$inFechaFin,$inActiva,$inDescripcionCupones,$inMultipleCantidad,$inClienteEspecial=null,$inLinkVideo=null,$inLinkVideo2=null,$inIdTipoServicio=null,$inHcc=null,$inGT=null,$inIdDescOpcion=null,$inLinkVideoGT=null,$inLinkVideoGT2=null,$inUbicaciones=null)             
{          
    global $connection;
	global $link;
    $error = "";
    ...
    if (!$result) $error = ErrorBd('al crear la campaña ');
...
function UpdateOferta($inId, $inIdDesc=null,$inTitulo=null, $inSubtitulo=null, $inDestacados=null, $inCondiciones=null, $inDescripcion=null, $inPrecioValor=null,$inDescuento=null,$inAhorro=null,$inPrecioFinal=null,$inDescripcionCupones=null,$inMultipleCantidad=null,$inLinkVideo = null,$inLinkVideo2 = null,$inIdTipoServicio=null,$inIdProvincias=null,$inIdDescOpcion=null,$inLinkVideoGT=null,$inLinkVideoGT2=null,$inUbicaciones=null)
{
    ...
    if ($result)
    {
		...
		if ($inLinkVideoGT!=null)
			$sql .=  ",link_video_GT = '".$inLinkVideoGT."'";
		if ($inLinkVideoGT2!=null)
			$sql .=  ",link_video2_GT = '".$inLinkVideoGT2."'";
...
function DeleteOfertasHist($inId) 
{
    ...
    if ($result) 
    {
        ...
        $result2 = Db::getInstanceOf()->ExecuteS($sql);
        $r2=$result2[0];
...
//    $sql .=  " ,test_descripcion = ".intval($activar)." desactivacion manual ".$desactivacion_manual." periodo ".$periodo." oferta: ".$idoferta;
        ...
        if ($result) 
        {
             foreach($result as $r)
             {
                ...
                //return ('active hist '.$r['active'].' desactmanual '.$desactivacion_manual. 'id_hist '.$r['id_oferta_hist']);
                 ...
                 $result_update = Db::getInstanceOf()->Execute($sql);
    ...
    return((!$result)?ErrorBd($error.'-'.$sql):false);
...
{ 
    ...
    {   //die(FuncionesOfertas::deleteImagen(intval($idoferta), $row['id_image_oferta']));    
        ...
        if (!FuncionesOfertas::deleteImagen(intval($idoferta), $row['id_image_oferta'],'oferta') OR !Db::getInstanceOf()->Execute('DELETE FROM `ps_image_oferta_lang` WHERE `id_image_oferta` = '.intval($row['id_image_oferta'])))
            return false;
    }
    return Db::getInstanceOf()->Execute('DELETE FROM `ps_image_oferta` WHERE `id_oferta` = '.intval($idoferta));       
  }
...
function deleteImagenesHistorico($idofertahist)
{
    ...
    {   //die(FuncionesOfertas::deleteImagen(intval($idoferta), $row['id_image_oferta']));    
        ...
        /*if(!Db::getInstanceOf()->Execute('DELETE FROM `ps_image_oferta_historico_lang` WHERE `id_image_oferta` = '.intval($row['id_image_oferta']))) return false;*/
    
    }
    return Db::getInstanceOf()->Execute('DELETE FROM `ps_image_oferta_historico` WHERE `id_oferta_hist` = '.intval($idofertahist));       
  }
