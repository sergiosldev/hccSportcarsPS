...
function GetCupones($inIdEstablecimiento=null,$inIdTalonario=null)
{
    ...
    {
        $sql = "SELECT fecha,numero,usado,vendido,observaciones,facturado FROM ps_cupones WHERE id_establecimiento = ".$inIdEstablecimiento." and id_talonario=".$inIdTalonario;
    ...
    while ($row = mysql_fetch_assoc($query))
    {
        ...
        $Cupon = new Cupon($inIdEstablecimiento, $inIdTalonario,$row['numero'], $row['usado'],$row['fecha'],$row['vendido'],$row['observaciones'],$row['facturado']);
        array_push($cuponArray, $Cupon);
    }
    return $cuponArray;
}
...
function UpdateCuponVendido($inIdEstablecimiento, $inNumTalonario,$inNumero,$inVendido)
{
    global $connection;
        ...
        $sql = $sql. " c.id_establecimiento = ".$inIdEstablecimiento." and t.id_talonario = ".$inNumTalonario." and c.numero = ".$inNumero;
