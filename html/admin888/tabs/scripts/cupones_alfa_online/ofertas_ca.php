...
include_once dirname(__FILE__).'/funciones_ofertas_vista_ca.php';             
//include dirname(__FILE__).'/../trazas.php';             

//include dirname(__FILE__).'/contador_periodos_oferta_ca.php';             
...
<?php
 if (request('creadas')==0)
{
    
    $ofertas = GetOfertasHistoricoCA(null,null,'activas_inactivas');
   // var_dump($ofertas);die;
    ...
    <div id="ofertas">  
        ...
        //  if ($o->clienteEspecial==1) $class='class="activo"';
        ?>
         ...
         <table <?echo($class);?>  width="100%"  style="border:1px solid #CCCCCC;margin-left:12px;width:950px;padding:7px;">
           ...
           <tr style="height:33px;vertical-align:top;" <?php echo($class);?>>   
               ...
               //echo('session '.$_SESSION['acceso_ofertas_ca_prestashop']);
               $url_detalle_oferta = $URL_ROOT.'detalle-oferta-dist/'.$o->id.'-0-oferta.html';
               $imagenes = $o->getImagenes();
               //echo('oferta '.$o->id);var_dump($imagenes);
               //traza('lista_imagenes_camp, id_oferta: '.$o->id.' nimages: '.count($imagenes));
               $nombre_imagen = '';
               foreach ($imagenes as $im) {if ($im['cover']) $nombre_imagen = $o->id.'-'.$im['id_image_oferta'].'.jpg';}
               //var_dump($imagenes);
               $size=80;
               list($Width, $Height, $tipo, $atr) = getimagesize(_PS_IMG_DIR_.'ohd/'.$nombre_imagen);
           ...
           <tr>
               <td width="10%" class="texto_oferta" style="text-align:left;vertical-align:top;padding-top:20px;"><img  <?php echo($w.' '.$h);?> src="../../../img/ohd/<?php echo($nombre_imagen);?>"></td>
               ...
               <td width="13%" class="texto_oferta">
                   ...
                   <input type="hidden" id="activa_ofertas<?php echo($i);?>" value = "<?php echo($o->activa);?>">
                    ...
                    ?>
                   <input type="hidden" id="fecha_ofertas_ca<?php echo($i);?>" value = "<?php echo($fecha_futura);?>">
                   <input type="hidden" id="idofertash_ca<?php echo($i);?>" value = "<?php echo($o->id);?>">
                   <input type="hidden" id="idofertas_ca<?php echo($i);?>" value = "<?php echo($o->idOferta);?>">
                   <input type="hidden" id="tiempo_limite_ca<?php echo($i);?>" >
                   <?php
                    if ($o->activa==1) 
                    {
               ...
               <td width="5%" class="texto_oferta valor_oferta">
                    <a title="Borrar la campaña" id="activa" href="javascript:borrar_oferta_ca('<?php echo($o->id);?>',0,<?php echo($ncupones);?>)">
           ...
           <!--<input class="boton" type="button" onclick="javascript:activar_talonarios(<?php echo($e->id);?>);" value='Mostrar ventas'>-->
               ...
               <td class="texto_ofrerta valor_oferta" style="text-align:left;vertical-align:top;padding-top:0;">
                    ...
                    <input type="button" title="Ver lista de cupones" class="boto"  value = 'Ver Lista' onClick="javascript:cupones_usados_ofertas_ca(<?php echo($o->id);?>,0)">
                    </td></tr>
                    <tr><td style="padding:7px;">    
						...
						<input type="button" value='Ver detalle' title="Ver detalle de la campaña"  class="boto" onClick="javascript:var ale='&gg'+(Math.floor(Math.random()*50000))+'=1';r=ajax_post.load('<?php echo $base_scripts_ca ?>/ajax_acceso_ofertas_prestashop.php?a=1'+ale);(document.getElementById('detalle<?php echo($o->id);?>')).click();">
        ...
        { 
            if ($o->orden == $maxorden)
            {
                ...
                if ($o->orden == $minorden)
                    ...
                    echo '[ <a  href="javascript:cambiar_posicion_oferta_ca('.$maxorden.',1,'.$o->id.');"><img src="../img/admin/down.gif" alt="" border="0"></a> ]';
            
            }
            elseif ($o->orden == $minorden) 
                echo '
                    [ <a href="javascript:cambiar_posicion_oferta_ca('.$o->orden.',0,'.$o->id.');"><img src="../img/admin/up.gif" alt="" border="0"></a> ]<br><br>
                    [ <img src="../img/admin/down_d.gif" alt="" border="0"> ]';
            else
                echo '
                    [ <a  href="javascript:cambiar_posicion_oferta_ca('.$o->orden.',0,'.$o->id.');""><img src="../img/admin/up.gif" alt="" border="0"></a> ]<br><br>
                    [ <a  href="javascript:cambiar_posicion_oferta_ca('.$o->orden.',1,'.$o->id.');""><img src="../img/admin/down.gif" alt="" border="0"></a> ]';
        }?>
           </td>    
           </tr>
...
else 
{
    ...
    $ofertas = GetOfertasCA(null,'orden');
    ...
    <div id="ofertas">  
        ...
        foreach($ofertas as $o)
          {
            ...
            $ofertasHist = GetOfertasHistoricoCA(null,$o->id,'activas');
            $url_detalle = '';
            foreach ($ofertasHist as $oh)
            {
                if ($oh->activa==1/* or $oh->clienteEspecial==1*/)
                {
        ...
        <td>
            <?php /*($o->clienteEspecial==1)?'activo':*/ ?>
            <table class="<?php echo($inactivo);?>" style="margin-left:10px;height:71px;border:1px solid #CCCCCC;padding:7px;width:100%;">               
               <tr>
        ...
        <td>
            <table  style="margin:0;margin-left:10px;border:1px solid #CCCCCC;padding:6px 0 6px 0;width:40px;">
               <tr>       
                   <td class="texto_cabecera" style="font-weight:bold;padding-right:16px;">Duplicar oferta</td>
                   <td class="texto_cabecera" style="font-weight:bold">Ofertas online</td>
               </tr>
               <tr>
                   <?php
                    if ($o->activa==1) 
                  ...
                  <!-- <td width="10%" align="left" style="padding:0 10px 0 10px;"> <input type="button" class="boto boton_marcar" onclick="javascript:marca_cliente_especial('<?php echo($o->id);?>','1','<?php echo($o->clienteEspecial);?>')"></td>--> 
                   <td width="1%" align="left" > <input type="button" class="boto boton_duplicar" onclick="javascript:duplicar_oferta_ca('<?php echo($o->id);?>')"></td> 
                   <td width="1%" align="left" > <input type="button" class="boto boton_duplicar" onclick="javascript:duplicar_ofertas_online_ca('<?php echo($o->id);?>')"></td> 
               </tr>
            </table>
         </td>
         </tr>
