...
function GetOfertasHistorico($inId=null,$inIdOferta=null,$orden=null,$clienteEspecial=null)
{
    ...
    while ($row = mysql_fetch_assoc($query))
    {
     //******
        ...
        $Oferta = new OfertaHistorico($row["id_oferta_hist"],$row["id_oferta"],$row["id_desc"], $row['name'], $row['subtitle'], $row['description_short'], $row["realiza"], $row['description'],$price, $row['reduction_percent'],$reduction_price,$final_price,$row['date_add'],$row['date_upd'],$row['active'],$row['observaciones'],$row['cliente_especial'],null,$row['orden'],$row['caducada'],$row['descripcion_cupones'],$row['multiple_cantidad']);
...
function InsertOferta($inIdDesc,$inTitulo, $inSubtitulo, $inDestacados, $inCondiciones, $inDescripcion, $inPrecioValor,$inDescuento,$inAhorro,$inPrecioFinal,$inDescripcionCupones,$inMultipleCantidad)
{          
    global $connection;
    $error = "";
    ...
    $sql = " insert into ps_oferta (id_oferta,orden,id_desc,id_category_default,price,reduction_percent,active,reduction_price,final_price,realiza,descripcion_cupones,multiple_cantidad) ";
 
    $sql = $sql. " values (".$NewId.",".$NewOrden.",'".$inIdDesc."',2,'".$inPrecioValor."','".$inDescuento."',0,'".$inAhorro."','".$inPrecioFinal."','".$inCondiciones."','".$inDescripcionCupones."','".$inMultipleCantidad."')";
...
function InsertOfertaHistorico($inIdOferta,$inIdDesc,$inTitulo, $inSubtitulo, $inDestacados, $inCondiciones, $inDescripcion, $inPrecioValor,$inDescuento,$inAhorro,$inPrecioFinal,$inFechaInicio,$inFechaFin,$inActiva,$inDescripcionCupones,$inMultipleCantidad)
{          
    global $connection;
    $error = "";
    ...
    $sql = " insert into ps_oferta_historico (id_oferta_hist,id_oferta,id_desc,id_category_default,price,reduction_percent,active,reduction_price,final_price,realiza,date_add,date_upd,orden,descripcion_cupones,multiple_cantidad) ";
 
    $sql = $sql. " values (".$NewId.",".$inIdOferta.",'".$inIdDesc."',2,'".$inPrecioValor."','".$inDescuento."',".$inActiva.",'".$inAhorro."','".$inPrecioFinal."','".$inCondiciones."','".$inFechaInicio."','".$inFechaFin."','".$NewOrden."','".$inDescripcionCupones."','".$inMultipleCantidad."')";
    //traza('debug.txt',$sql);
    //return array(1,$sql);
    $result=mysql_query($sql);
...
function UpdateOferta($inId, $inIdDesc=null,$inTitulo=null, $inSubtitulo=null, $inDestacados=null, $inCondiciones=null, $inDescripcion=null, $inPrecioValor=null,$inDescuento=null,$inAhorro=null,$inPrecioFinal=null,$inDescripcionCupones=null,$inMultipleCantidad=null)
{
...
function DeleteOfertasHist($inId) 
{
    ...
    $sql = " select id_oferta from ps_oferta_historico where id_oferta_hist = '".$inId."'";
    $result = mysql_query($sql);
    ...
    if ($result) 
    {
        ...
        {
            $sql = " update ps_oferta set active=0 where id_oferta = '".$r['id_oferta']."'";  
