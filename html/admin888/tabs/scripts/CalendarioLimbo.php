...
{
echo '<span class="calendario"><a href="javascript:ano_limbo(\'-\')">  <img src="'.URL_ROOT.'img/esquerra.gif"  alt="" /> </a>'.$ano.'<a href="javascript:ano_limbo(\'+\')">  <img src="'.URL_ROOT.'img/dreta.gif"  alt="" /> </a></span>'; // COLOCAR EL MES
...
$dies_ocupats=$this->dies_ocupats();	

$_MESES['cas']=array('Enero','Febrero','Marzo','Abril','Mayo','Junio','Julio','Agosto','Septiembre','Octubre','Noviembre','Diciembre');

echo '<span class="calendario"><a href="javascript:mes_limbo(\'-\',ciudad_aux_limbo,tipus_ev_limbo)">  <img src="'.URL_ROOT.'img/esquerra.gif"  alt="" /> </a> '.strtoupper($_MESES['cas'][$mes-1]).' <a href="javascript:mes_limbo(\'+\',ciudad_aux_limbo,tipus_ev_limbo)">  <img src="'.URL_ROOT.'img/dreta.gif"  alt="" /> </a> </span>'; // COLOCAR EL MES
...
for($i=1;$i<=$this->devolver_diasmes($mes,$ano);$i++) 
    { 
	 ...
	 if(in_array($ano.'-'.$aux_mes.'-'. $aux_i,$this->dies_ocupats()))
	 {
		 	if(in_array($ano.'-'.$aux_mes.'-'. $aux_i,$this->dies_ocupats_color()))
		 	{
				$marcat='no_marcat'; $click='onclick="click_dia(\''.$ano.'-'.$aux_mes.'-'. $aux_i.'\')"'; 
			}
			else if($this->dies_ocupats_lila($ano.'-'.$aux_mes.'-'. $aux_i))
			{
			    $marcat='marcat_lila'; $click='onclick="click_dia(\''.$ano.'-'.$aux_mes.'-'. $aux_i.'\')"'; 
            }
            else if($this->dies_ocupats_gris($ano.'-'.$aux_mes.'-'. $aux_i))
            {
