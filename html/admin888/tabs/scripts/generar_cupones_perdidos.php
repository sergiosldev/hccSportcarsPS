...
/*    $cupon_oferta = new CuponOferta($codigo_cupon,$codigo_reserva,$_SESSION['id_oferta'],$id_usuario,$descripcion, $fechaIni, $fechaFin,0,1,'',null,$transaccion,null,$fecha_compra,null,$ipn,$pdt);  
    $ins=$cupon_oferta->Insert();
...
*/
        ...
        //var_dump('ofertahists2 '.$re[0]['id_oferta_hist']);die;
            $ins=$cupon_oferta_hist->Insert();
            //echo($ins);die;
        
            $result = ($ins==='0')?false:true;
            if($result){
                //envia_mails($usuario,$cupon_oferta);
                unset($cupon_oferta);
...
function marca_reserva_usuario($reserva){
 ...
 //echo('reserva: '.$reserva->id.' resultado_modif_marca '.$r);
 return $r;
 }     
?>