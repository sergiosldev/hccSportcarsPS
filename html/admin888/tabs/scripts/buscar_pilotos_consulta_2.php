...
include_once(dirname(__FILE__).'/../../../config/defines.inc.php');
 ...
 while($r=mysql_fetch_object($result))
	 ...
	 echo '<td class="columna" style="border-bottom:1px solid #ccc" >'.$link_reserva.strtoupper ($ciudad).'</a>'; 
	 echo '</td>';
...
//	 echo '<td style="border-bottom:1px solid #ccc" >'.preg_replace("/(".strtoupper (convert_caracters_hex(trim($_REQUEST['nombreb']))).")/",'<span style="color:#f00" >\1</span>',strtoupper (trim($r->persona_regala)));
	...
	//PaginaciÃƒÂ³ Generada segons el resultat de la consulta, els numeros han de dur a js per canviar el contingut per ajax/DOM.
			...
			if ($pageNum != 1)
				...
				for ($i=1;$i<=$total_paginas;$i++) {
					if ($pageNum == $i)
							...
							//si el ÃƒÂ­ndice no corresponde con la pÃƒÂ¡gina mostrada actualmente,
							//coloco el enlace para ir a esa pÃƒÂ¡gina
	...
	$imprimir ="	
	  <script>
			function canviar_pag(page) {	 
			  ...
			  xhttp.open('POST','". _BASE_URL_  ."/admin888/tabs/scripts/buscar_pilotos_ajax.php', true);
			  xhttp.send('q='$sql2'&p='+page);
			}
	  </script>
	";
  ...
  /**
  * FunciÃƒÂ³ per convertir els caracters especials a codi hexadecimal que podem mostrar.
  *@param $cad Cadena que contÃƒÂ© la informaciÃƒÂ³ a transformar.
 ...
 function convert_caracters_hex($cad)
  ...
  /**
  * FunciÃƒÂ³ per identificar el vehicle a partir d'una variable.
  *@param $t 
  ...
  */
  ...
  /**
  * FunciÃƒÂ³ per identificar les experiencies a partir d'un vehicle d'una variable.
  *@param $t
  ...
  */
  ...
  {
    $originales =  'Ãƒâ‚¬Ãƒï¿½Ãƒâ€šÃƒÆ’Ãƒâ€žÃƒâ€¦Ãƒâ€ Ãƒâ€¡ÃƒË†Ãƒâ€°ÃƒÅ Ãƒâ€¹ÃƒÅ’Ãƒï¿½ÃƒÅ½Ãƒï¿½Ãƒï¿½Ãƒâ€˜Ãƒâ€™Ãƒâ€œÃƒâ€�Ãƒâ€¢Ãƒâ€“ÃƒËœÃƒâ„¢ÃƒÅ¡Ãƒâ€ºÃƒÅ“Ãƒï¿½ÃƒÅ¾ÃƒÅ¸ÃƒÂ ÃƒÂ¡ÃƒÂ¢ÃƒÂ£ÃƒÂ¤ÃƒÂ¥ÃƒÂ¦ÃƒÂ§ÃƒÂ¨ÃƒÂ©ÃƒÂªÃƒÂ«ÃƒÂ¬	ÃƒÂ®ÃƒÂ¯ÃƒÂ°ÃƒÂ±ÃƒÂ²ÃƒÂ³ÃƒÂ´ÃƒÂµÃƒÂ¶ÃƒÂ¸ÃƒÂ¹ÃƒÂºÃƒÂ»ÃƒÂ½ÃƒÂ½ÃƒÂ¾ÃƒÂ¿Ã…â€�Ã…â€¢';
