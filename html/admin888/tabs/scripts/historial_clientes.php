...
include_once dirname(__FILE__).'/../../../classes/CuponOferta.php'; 
...
foreach ($cupones as $cupon)   
{
    ...
    if ($idOfertaHist!='' and ($idOfertaHist != $cupon->idOfertaHist or $fechaCompra!=implode('/',array_reverse (explode('-',substr($cupon->fechaCompra,0,10)))).substr($cupon->fechaCompra,10)) )
    {
        ...
        $historial[$i]['transacciones'] = implode('<br>',$transacciones);
        $historial[$i]['empresas'] = implode('<br>',$empresas);
        $historial[$i]['venta'] = $i+1;
        $historial[$i]['id_oferta_hist'] = $idOfertaHist;
		$historial[$i]['id_opcion_oferta'] = $idOpcionOferta;                                            
    ...
    if ($idOfertaHist=='') 
    {
		...
		$idOpcionOferta = $cupon->idOpcionOferta;
    ...
    $transacciones[]=$cupon->transaccionCompra;    

	switch($cupon->empresa)
	{
		case 2:
			$empresas[]=2;
			break;
    ...
    foreach($historial as $r)
      {
        ...
        $acupones = explode('<br>',$r['codigos']);
        $atransacciones = explode('<br>',$r['transacciones']);
        $aempresas = explode('<br>',$r['empresas']);
		$aempresas = array_map('desc_empresa',$aempresas);
		//var_dump($aempresas);
        $atmp=array_map('concatenar',$acupones,$atransacciones);
        $atmp=array_map('concatenar',$aempresas,$atmp);
        $cupones_transac=implode('<br>',$atmp); 
                //$cadena_cupones = '\''.str_replace('<br>','\',\'',$r['codigos']).'\'';
        $cadena_cupones = '\''.str_replace('<br>',',',$r['codigos']).'\'';
		
		//echo('cad_cupones '.$cadena_cupones);
        //$cadena_transacciones = '\''.str_replace('<br>',',',$r['transacciones']).'\'';    		
    ?>
     <table width="100%"  style="border:1px solid #CCCCCC;margin-left:12px;width:960px;padding:7px;">
       <tr style="height:33px;vertical-align:top;">   
           <td class="cabecera_cliente" width="<?php echo($ancho_columna[0]);?>%;">Códigos (Cupón-Transacción)</td> 
       ...
       <tr>
           <td width="<?php echo($ancho_columna[0]);?>%;" class="texto_cliente valor_cliente"><a href="javascript:envia_formulario_cupon_oferta('<?php echo($r['codigos']);?>');"><?php echo($cupones_transac);?></a></td>
           ...
           <td width="<?php echo($ancho_columna[5]);?>" style="padding:7px 0 7px 0;text-align:center;">
               <?php //echo($cadena_cupones);?>
                <?php if (trim($r['id_opcion_oferta'])!='') { ?>
				<input style="width:70px;<?php if ($ncupones==0) echo('visibility:hidden;');?>" type="button" title="Reenviar Cupones" class="boto"  value = 'Reenviar' onClick="javascript:document.body.style.cursor='wait';envia_cupones(<?php echo($id_usuario);?>,<?php echo($r['id_oferta_hist']);?>,<?php echo($cadena_cupones);?>,0,<?php echo($r['id_opcion_oferta']);?>);document.body.style.cursor='default';">
				<?php } else  {?>
				<input style="width:70px;<?php if ($ncupones==0) echo('visibility:hidden;');?>" type="button" title="Reenviar Cupones" class="boto"  value = 'Reenviar' onClick="javascript:document.body.style.cursor='wait';envia_cupones(<?php echo($id_usuario);?>,<?php echo($r['id_oferta_hist']);?>,<?php echo($cadena_cupones);?>);document.body.style.cursor='default';">
				<?php } ?>
		   </td>
       </tr>
