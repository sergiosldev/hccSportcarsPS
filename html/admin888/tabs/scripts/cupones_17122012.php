...
foreach ($cupones as $n) {if ($n->vendido==1) $vendidos+=1;if ($n->facturado==1) $facturados+=1;}
...
<div id="cupones">  
    <table>
        <tr class="cabecera">
            <td width='10%'></td>
            <td style="width:20%;"><span class="etiqueta_">Vendidos</span></td><td style="width:20%;"><span class="etiqueta_">Validados</span></td>
            <td style="width:20%;"><span class="etiqueta_">Pendientes</span></td>
            <td><span class="etiqueta_">Facturados</span></td>
            <td><span class="etiqueta_">Nº Talonario</span></td>
            <td><span class="etiqueta_"></span></td>
        </tr>
        <tr>    
     <!--<td style="width:20%;text-align:left;"><input type="button"  onclick="javascript:buscar_cheque(<?php echo($id_establecimiento);?>)" value="Validar"></td> -->
          <td width='10%'></td>
          <td style="width:20%;"><input class="input" type="text" style="text-align:center;width:60px;" disabled value="<?php echo($vendidos);?>"></td>
          <td style="width:20%;"><input class="input" type="text" style="text-align:center;width:60px;" disabled value="<?php echo($validados);?>"></td>
          <td style="width:15%;"><input class="input" type="text" disabled  style="text-align:center;width:60px;" value="<?php echo($pendientes);?>"></td>
          <td style="width:15%;"><input class="input" type="text" disabled  style="text-align:center;width:60px;" value="<?php echo($facturados);?>"></td>
          <td style="width:15%;"><input class="input" type="text" disabled  style="text-align:center;width:60px;" value="<?php echo($numero_talonario);?>"></td>
        </tr>
    </table>    
    ...
    foreach($cupones as $c)
      ...
      {        
    ?>
      <tr><td colspan="11" style="text-align:right;color:#555555;font-weight:bold;font-size:11px;">Facturado</td></tr>
    <?php 
      } ?>  
      <tr>
       <td width='10%'></td>
       <td width='12%' ><?php echo($i." - ");?></td>
       <td width="16%" class="borderg"><?php printf("%06d", $c->numero); ?></td>
       <td class="registro_cupon">
            ...
            <?php //mts 12052012.  Sólo activaremos la función de validación para los cupones vendidos o validados. 
               if($c->vendido==1 or $c->usado==1 or $c->usado==3) { ?> 
                <a title="<?php echo($titulo);?>" style="<?php echo(($c->usado==1)?'font-size:10px;color:#FFFFFF;':'');?>display:block;width:105px;height:17px;text-align:center;" onmouseover="this.style.backgroundcolor='#FFFFFF;'" class="<?php echo($class);?>" href="javascript:validar_cupon(<?php echo($c->id_establecimiento);?>,<?php echo($numero_talonario);?>,<?php echo($c->id_talonario);?>,<?php echo($c->numero);?>,<?php echo($c->usado==0?'0':'1');?>)">
            <?php } else {?>
                <a title="<?php echo($titulo);?>" style="<?php echo(($c->usado==1)?'font-size:10px;color:#FFFFFF;':'');?>display:block;width:105px;height:17px;text-align:center;"  onmouseover="this.style.backgroundcolor='#FFFFFF;'" class="<?php echo($class);?>" href="javascript:">
            <?php } ?>    
            <?php if ($c->usado==0 or $c->usado==3)  {?>
              <?php echo(($c->usado==3)?"Cancelado":"");?>
       ...
       <td style="padding-left:10px;font-weight:bold;width:0%;" class="registro_cupon"></td>
       <td class="registro_cupon" style="padding-right:24px;">
                <a href="javascript:validar_cupon(<?php echo($c->id_establecimiento);?>,<?php echo($numero_talonario);?>,<?php echo($c->id_talonario);?>,<?php echo($c->numero);?>,<?php echo($c->facturado?5:4);?>)" id="facturado" title="Cupón <?php echo($c->facturado?"":"no");?> facturado" >
                        <img style="width:20px;height:20px;" src="../../../img/<?php if ($c->facturado) echo('facturado.png'); else echo('no_facturado.png');?>">
                </a>    
       </td>
       <td></td>
       </tr>
