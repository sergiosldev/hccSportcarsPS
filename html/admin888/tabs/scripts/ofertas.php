...
include_once dirname(__FILE__).'/funciones_cupon_oferta_historico.php'; 
include_once dirname(__FILE__).'/funciones_ofertas_vista.php';             
include_once dirname(__FILE__).'/contador_periodos_oferta.php';             

$URL_ROOT='https://'.$_SERVER['HTTP_HOST'].__PS_BASE_URI__ ;
...
if ($_GET['creadas']==0)
{
	...
	//echo('camp '.$_GET['filtro_camp'].' serv '.$_GET['tipo_servicio']);
   
    $ofertas = GetOfertasHistorico(null,null,'activas_inactivas',null,$_GET['filtro_camp'],null,$_GET['tipo_servicio']);
	/*if (tools::getuserip()=='95.23.12.64')
	{
   ...
   // var_dump($ofertas);die;
    ...
    $filtrocamp = intval($_GET['filtro_camp']);
	?>
    ...
    <div id="ofertas">  
        ...
        $texto_periodo_automatico = ($o->periodoAutomatico==1)?"A":"";
	   ...
	   $empresa = '';
	   $sep=' / ';
	   if ($o->hcc==1) {$hcc=1;$empresa.=(($empresa=='')?'':$sep).'HCC';}
	   if ($o->GT==1) {$GT=1; $empresa.=(($empresa=='')?'':$sep).'GT';}
         ...
         <table <?echo($class);?>  width="100%"  style="border:1px solid #CCCCCC;margin-left:12px;width:950px;padding:7px;">
           ...
           <tr style="height:33px;vertical-align:top;" <?php echo($class);?>>   
              ...
              //var_dump($o)
			   ...
			   $ymd=explode('-',substr($o->fechaAlta,0,10));
               $y=$ymd[0];
			   $m=$ymd[1];
			   $d=$ymd[2];
               ?>
           ...
           <tr>
               <td width="10%" class="texto_oferta" style="text-align:left;vertical-align:top;padding-top:20px;" text="Campaña número<?php echo($o->idOfertaHist);?>"><img  <?php echo($w.' '.$h);?> src="../../../img/oh/<?php echo($nombre_imagen);?>"></td>
               <td width="15%" class="texto_oferta" style="text-align:left;">
                   <table>
                       <tr><td  style='font-weight:bold;color:#3399FF;'><?php echo(utf8_encode($o->idDesc).' ['.$empresa.'] ('.$o->id.')');?></td></tr>
               ...
               <td width="13%" class="texto_oferta">
                   <input type="hidden" id="periodo_automatico_ofertas<?php echo($i);?>" value = "<?php echo($o->periodoAutomatico);?>">
                   ...
                   <?php
					...
					?>
				   <input type="hidden" id="tiempo_limite<?php echo($sufijo.$i);?>" >
                   <?php
                    if ($o->activa==1) 
                    {
                   ...
                   ?>
                   <div id="titulo_estado_oferta<?php echo($i);?>" class="<?php echo($clase_activa);?>"><span style="margin:5px 0 auto;width:33px;display:block;text-align:center;font-weight:bold;color:#ffffff;"><?php echo($texto_periodo_automatico);?></span></div>
...
{
    $ofertas = GetOfertas(null,'orden',$_GET['tipo_oferta'],$_GET['tipo_servicio']);
    ...
    <div id="ofertas">  
        ...
        foreach($ofertas as $o)
          {
			...
			$ofertasHist = GetOfertasHistorico(null,$o->id,'fecha');
            $url_detalle = '';
            foreach ($ofertasHist as $oh)
            {
                if ($oh->activa==1)
                {
					...
					$url_detalle = 'detalle-oferta/'.$ofertasHist[0]->id.'-0-oferta.html';
                    break;
                }
        ...
        <td>
            <table class="<?php echo(($o->clienteEspecial==1)?'activo':$inactivo);?>" style="margin-left:10px;height:71px;border:1px solid #CCCCCC;padding:7px;width:98%;">
               <tr>
                   <?php
                   $precio=floatval($o->precioValor);
                   if (floatval($o->descuento)!=0) $precio_oferta = floor(floatval($precio-$precio*$o->descuento/100));
				   ...
				   if ($o->hcc==1) {$hcc=1;$empresa.=(($empresa=='')?'':$sep).'HCC';}
				   if ($o->GT==1) {$GT=1; $empresa.=(($empresa=='')?'':$sep).'GT';}
				   if ($o->periodoAutomatico==1) $periodo_automatico=1;
				   else $periodo_automatico=0;		
				   if ($empresa!='') $empresa = ' ['.$empresa.']';	
                   
				   if ($o->clienteEspecial==1)
				   {
					   /*if ($o->motorclub==1) {$url_detalle='https://www.motorclubexperience.com/'.$url_detalle;}   
					   else if ($o->dreamcars==1) {$url_detalle='https://www.dreamcarsexperience.com/'.$url_detalle;}
					   else if ($o->hcc==1) {$url_detalle='https://www.hccsportcars.com/'.$url_detalle;}
					   else $url_detalle = $URL_ROOT.$url_detalle;
					   */
					   $url_detalle=_BASE_URL_.$url_detalle;
				   }
				   else
					{
						$url_detalle = $URL_ROOT.$url_detalle;
					}
                  ...
                  ?> 
                   ...
                   <div id="titulo_estado_oferta<?php echo($i);?>" class="<?php echo($clase_activa);?>" style="margin-left:0px;color:#ffffff;"><span style="margin:5px 0 auto;width:33px;display:block;text-align:center;font-weight:bold;"><?php echo($texto_periodo_automatico);?></span></div>
                   </td>

                   <td width="200px"><span class="texto_cabecera" style="height:30px;width:200px;">Id Oferta</span> <span class="texto_normal"><?php echo(utf8_encode($o->idDesc).' - '.$empresa. '('.$o->id.')');?></span></td>
                   <td width="184px" align="left"> <input type="button"  style="width:70px;height:30px;" class="boto"  text="Editar oferta"  onclick="javascript:edita_menu_oferta('<?php echo($o->id);?>','<?php echo($periodo);?>',0,'<?php echo($o->clienteEspecial);?>',<?php echo($hcc);?>,<?php echo($GT);?>,<?php echo($periodo_automatico);?>);" value="Editar"></td>
        ...
        <td>
            <table  style="margin:0;margin-left:0px;border:1px solid #CCCCCC;padding:6px 0 6px 0;width:133px;">
               <tr>       
                   <td class="texto_cabecera" style="font-weight:bold;width:50px;padding:0 10px 0 10px;">Vip</td>
                   <td class="texto_cabecera" style="font-weight:bold;padding-right:2px;">Dupl. oferta</td>
                   <td class="texto_cabecera" style="font-weight:bold">Cup. Alfa </td>
               </tr>
               <tr>
                   <?php
                    if ($o->activa==1) 
                   ...
                   ?>
                   <td width="10%" align="left" style="padding:0 5px 0 5px;"> <input type="button" class="boto boton_marcar" onclick="javascript:marca_cliente_especial('<?php echo($o->id);?>','1','<?php echo($o->clienteEspecial);?>')"></td> 
                   <td width="10%" align="left" > <input type="button" class="boto boton_duplicar" onclick="javascript:duplicar_oferta('<?php echo($o->id);?>')"></td>
                   <td width="1%" align="left" > <input type="button" class="boto boton_duplicar" onclick="javascript:copiar_oferas_a_cupones_online('<?php echo($o->id);?>')"></td> 
               </tr>
            </table>
         </td>
         </tr>
