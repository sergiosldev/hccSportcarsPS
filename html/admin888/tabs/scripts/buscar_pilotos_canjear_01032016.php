...
';
 echo($listbox);
 echo('<input type="button" value="Canjear" onclick="canjear();" style="float:left;margin-left:10px;margin-top:10px;"/></div>');	
...
$k=0;
 while($r=mysql_fetch_object($result))
	 ...
	 $link_canjear='<a href="javascript:marcar_canjeado(\''.$r->nombre_origen.'\',\''.$sciudad.'\',parseInt($(\'#marcado'.$r->id.'\').val()),'.$r->id.')">';    
	 ...
	 echo '<td style="display:none;"><input type="hidden" id="visible'.$r->id.'" value="'.intval($svisible).'"></td>';      
	 echo '<td style="display:none;"><input type="hidden" id="ciudad'.$r->id.'" value="'.$sciudad.'"> '.$link_canjear.$sciudad.'</a></td>';    
	 echo '<td class="columna" style="border-bottom:1px solid #ccc" >'.$link_marcar_no_visible.'</td>';
	 echo '<td class="columna" style="border-bottom:1px solid #ccc" >'.$link_canjear.$sciudad.'</a></td>';  
	 echo '</a></td>';
 
	 echo '<td  class="columna" style="color:#00f;background:#ff8;font-weight:bold">'.$link_canjear.$r->id_event.'</a></td>';   
