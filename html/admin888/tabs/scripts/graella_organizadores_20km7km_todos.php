...
//TIPO
	...
	else if($tipoev=='_bferrari_' || $tipoev=='_blamborghini_'  || $tipoev=='_bporsche_' || $tipoev=='_bcorvette_' )
	  include 'dies_graella4.php';
	else
	{  
...
//TIPO BAUTIZO
	...
	else if($tipo_bautizo=='_bferrari_' || $tipo_bautizo=='_blamborghini_' || $tipo_bautizo=='_bporsche_' || $tipo_bautizo=='_bcorvette_')
	  include 'dies_graella4.php';
	else  
...
/*
  ...
  */
  ...
  elseif($tipus=='_porsche_'  || $tipus=='porsche')
  {
	/*if ($mati=='#')
		$t_aux='(i.tipus_event="_porsche_") ';
	else*/
		$t_aux='(i.tipus_event="porsche" OR i.tipus_event="_porsche_" OR i.tipus_event="_bporsche_") ';
  }

  elseif($tipus=='_corvette_'  || $tipus=='porsche')
  {
	/*if ($mati=='#')
		$t_aux='(i.tipus_event="_porsche_") ';
	else*/
		$t_aux='(i.tipus_event="_corvette_" OR i.tipus_event="corvette" OR i.tipus_event="_bcorvette_") ';
  }
...
// fi graelles dobles
	...
	$texto_bautizo='';
    if($numr=mysqli_num_rows($result)) { // plaza ocupada 
	...
	*/
	 ...
	 if(strtoupper(trim(substr($r['tipus_event'],0,2)))=='_B' && strtoupper(trim($r['tipus_event']))!='_BUGGY_')          
	 {
		 ...
		 if ($r['ruta']==14 or strpos(strtolower($r['observaciones']),'14km')!==false or  strpos(strtolower($r['observaciones']),'14 km')!==false )
		 {
			$texto_bautizo='B14';
		 }
		 else
	 ...
	 else 
	 {
	  ...
	  if(trim($r['email']).trim($r['email_confirm']).trim($r['email_persona_regala']).trim($r['persona_regala'])!='')
	   {
			if ($texto_bautizo=='BAU')
			{
				...
				{
						 if ($r['ruta']==40 or strpos(strtolower($r['observaciones']),'40km')!==false or  strpos(strtolower($r['observaciones']),'40 km')!==false )
						 {
							$texto_ruta='R40';
						 }	
						 else
					...
					$cad='<div  id="disponibilidad"  class="no_disponible_carbassa cuadro"    >'.$texto_ruta.'&nbsp;&nbsp;&nbsp;&nbsp;</div>';
				}
	   }
	  ...
	  if(trim(substr($r['tipus_event'],0,2))=='_b' && strtoupper(trim($r['tipus_event']!='_BUGGY_')))
	  {
		if (trim($r['email_confirm'])!='')
		{
		 if ($r['ruta']==14 or strpos(strtolower($r['observaciones']),'14km')!==false or  strpos(strtolower($r['observaciones']),'14 km')!==false )
		 {
			$cad='<div   id="disponibilidad"  class="marca_bautizo cuadro" style="height:15px;" >R14</div>';
		 }
		 else
		 {
			$cad='<div   id="disponibilidad"  class="marca_bautizo cuadro" style="height:15px;" ></div>';         
      ...
      $sobservaciones=$r['Observaciones'];

	  if ((intval(strpos($sobservaciones,'-'))==intval(strlen($sobservaciones)-1))) $sobservaciones=substr($sobservaciones,0,-1);
      $sobservaciones=str_replace('OTORCLUBEXPERIENCE','OTORCLUB',$sobservaciones);
	  $sobservaciones=str_replace('CARSEXPERIENCE','CARS',$sobservaciones);

	  $sorigen=$r['origen'];
      $sorigen=str_replace('OTORCLUBEXPERIENCE','OTORCLUB',$sorigen);
	  $sorigen=str_replace('CARSEXPERIENCE','CARS',$sorigen);	 	 
	  ...
	  $info='
        ...
        <td  style="width:100px;font-size:10px;vertical-align:middle;border-top:1px solid #000000;border-bottom:1px solid #000000;'.$border_style.'" class="borderg " align="left" > '.utf8_encode($sobservaciones).' </td>
			...
			<table style="border:none;">
			  <tr>
				...
				<td style="width:50px;">'; 
				 ...
				 if($r['marcat']=='1')
				 {
					 ...
					 $info.='<div style="background: none repeat scroll 0% 0% '.$color.'; border: 0px solid #000; padding: 2px 8px;width:8px;">&nbsp;&nbsp;&nbsp;&nbsp;</div>';             
			  ...
			  <tr>
				...
				<td style="width:60px;text-align:left;">
					<span style="font-weight:bold;color:#2222FF;">'.$sorigen.'</span>
				</td>
			 </tr>
			 </table>
			</td>';
...
//AFEGIR FILES EXTRES, 4 CAMPS.
function files_extres_pdf()
{
echo('
<table border="1" cellspacing="0" cellpadding="0" style="width:1200px;padding-top:10px;padding-right:5px;border-spacing:0 14px;">
	<tr>
		<td align="right" class="info extra" style="font-size:9px;vertical-align:middle;padding-right:40px;">
			<span>Hora:&nbsp;&nbsp;&nbsp;&nbsp; &nbsp;&nbsp;&nbsp;&nbsp; </span>
		</td>		
		<td height="100px" width="250px" align="right" class="info extra" style="font-size:9px;vertical-align:middle;padding-right:210px;">
			<span>Piloto:</span>
		</td>
		<td width="250px" align="right" class="info extra" style="font-size:9px;vertical-align:middle;padding-right:210px;">
			<span>Plataforma:</span>
		</td>
		<td width="250px" align="right" class="info extra" style="font-size:9px;vertical-align:middle;padding-right:100px;">
			<span>Ruta:</span>
		</td>
	</tr>
</table>
