...
if ($_GET['creadas']==0) //creadas == 0 indica el registro de ofertas (histórico).
{
    ...
    if (trim($id_oferta)=='' and trim($_GET['cupon'])!='') 
	{
...
<div id="cupones">  
	...
	?>
      ...
      <tr <?php if (isset($_GET['cupon'])) {if ($_GET['cupon']==$c->cupon || $_GET['cupon']==$c->transaccionCompra){echo('style="background: #ffc0c0;"');} }?>>                            
       <td width='2%'>
	   <?php 
		if (isset($_GET['cupon'])) 
		{
			if ($_GET['cupon']==$c->cupon || $_GET['cupon']==$c->transaccionCompra)        
			{
			?>
