...
if (tools::getValue('guardar_empresas'))
{
	...
	if (tools::getValue('motorclub')==1) $motorclub = 1; 
	if (tools::getValue('dreamcars')==1) $dreamcars = 1; 
	if (tools::getValue('hcc')==1) $hcc = 1; 
	if (tools::getValue('periodo_automatico')==1) $pa = 1; 
	else $pa=0;
	
	$r=$oferta->UpdateCampo('motorclub',$motorclub,tools::getValue('id_oferta'));
	$r=$oferta->UpdateCampo('dreamcars',$dreamcars,tools::getValue('id_oferta'));
    ...
    foreach ($ohrs as $ohr) {
        if (!$ohr || !method_exists($ohr, 'UpdateCampo')) {
            error_log('Elemento nulo en ohrs o sin método UpdateCampo');
            continue;
        }
...
if (tools::getValue('duplicar')=='1')
{
	...
	$sdescripcion = mysql_real_escape_string($o->descripcion);
    $scondiciones = $o->condiciones;   
    $sdescripcionCupones = $o->descripcionCupones;    

    //echo('destacados '.$sdestacados);die;
    $ret=InsertOferta($o->idDesc,$o->titulo, $o->subtitulo, $sdestacados,$scondiciones, $sdescripcion, $o->precioValor,$o->descuento,$o->ahorro,$o->precioFinal,$o->descripcionCupones,$o->multipleCantidad,$o->linkVideo,$o->linkVideo2,$o->idTipoServicio,null,$o->linkVideoDreamcars,$o->linkVideo2Dreamcars,$o->linkVideoHcc,$o->linkVideo2Hcc);                                                                                                                                      
...
*/
       ...
       {     
           ...
           $cad_eval.='id_(\'link_video_oferta_dreamcars\').value=\''. $r->linkVideoDreamcars.'\'; ';    
           $cad_eval.='id_(\'link_video_oferta_dreamcars2\').value=\''. $r->linkVideo2Dreamcars.'\'; ';    
           $cad_eval.='id_(\'link_video_oferta_hcc\').value=\''. $r->linkVideoHcc.'\'; ';    
           $cad_eval.='id_(\'link_video_oferta_hcc2\').value=\''. $r->linkVideo2Hcc.'\'; ';    
