...
function envia_formulari_oferta(idoferta,num_opciones,boton)
  {
	...
	$('#ubicaciones').val(tinyMCE.get("ubicaciones").getContent());
...
//    dades+='&nlineas_descripcion='+nlineas_descripcioncupones;
  ...
  {
    if (id_('cliente_oferta').value=='') {alert('Debe introducir algún criterio de búsqueda para la oferta');return;}
  ...
  function envia_formulario_baja_clientes()
  {
    var arr_res;
	email=id_('cliente_baja').value;
	
	if (id_('cliente_baja').value=='') {alert('Debe introducir algún criterio de búsqueda');return;}
       
    var aleatorio='&gg'+(Math.floor(Math.random()*50000))+'=1';
    if (id_('cliente_baja').value==undefined) 
		dades='email=';
	else 
		dades = 'email='+email; 

	//alert(dades);
    r=ajax.load('<?php echo $base_scripts ?>ajax_comprobar_cliente_baja.php?'+dades+aleatorio);  
    id_('lista_baja_clientes').style.display='block';
	arr_res=$.trim(r).split('#');
	if (arr_res[0]==0)
	{
		alert('No existen usuarios ni reservas para este email');
	}
		
	else 
	{
		if(confirm(arr_res[1]+' desea eliminarlos?'))
		{
			id_('frm_borrar_baja_clientes').reset();
			id_('email_ba').value=email;
			$.colorbox({width:"42%", inline:true, href:"#form_borrar_baja_clientes",open:true});   
		};
	}
	//id_('lista_baja_clientes').innerHTML = r;  
	//ocultar_form_busqueda_baja_clientes();
    //$.colorbox({width:"80%", inline:true, href:"#lista_clientes",open:true}); 
 ...
 function envia_formulario_borrar_cliente_baja(email,password)
 {
	...
	r=ajax.load('<?php echo $base_scripts ?>ajax_borrar_cliente_baja.php?'+dades+aleatorio);  
    //alert(r);
    if (r.indexOf('error_password')!=-1) {alert('Contraseña incorrecta. Inténtelo de nuevo.'); id_('frm_borrar_baja_clientes').reset();}
    else 
    {
		alert('Los usuarios y reservas asociadas a este email se han eliminado');
		cbclose();
	}
 } 
...
function marca_cliente_especial(id_oferta,creadas,marcado)
{
 ...
 function activar_oferta(id_oferta,activar,periodo,tipo,cliente_especial,hcc,GT,periodo_automatico)
 {
  ...
  if (activar)
  {
	  ...
	  /*switch(periodo)
	  {
		...
		if (tmp_hcc!= undefined) hcc=tmp_hcc;
		if (tmp_GT!=undefined) GT=tmp_GT;
		if (hcc==1) {id_('hcc').checked = true;id_('menu_hcc').value=1;id_('menu_hcc').checked=true;}
		if (GT==1) {id_('GT').checked = true;id_('menu_GT').value=1;id_('menu_GT').checked=true;}
 ...
 function envia_formulario_activar_oferta(id_oferta,periodo,activar,tipo,hcc,gt)
 {
    var ale='&gg'+(Math.floor(Math.random()*50000))+'=1';

    id_('msg_error').innerHTML='';
    
    r=ajax.load('<?php echo $base_scripts ?>ajax_ofertas_bd.php?idoferta='+id_oferta+'&periodo='+periodo+'&activar='+activar+'&edicio_oferta='+(activar?'activar':'desactivar')+'&tipo='+tipo+'&hcc='+hcc+'&gt='+gt+ale); 
    if (r.indexOf('Error')!=-1) 
    {   
        $.colorbox.close();
        alert(r);
...
//de forma autom�tica.
 function enviar_formulario_guardar_empresa(id_oferta,hcc,gt,periodo_automatico,show_message)
 {
    var ale='&gg'+(Math.floor(Math.random()*50000))+'=1';

    id_('msg_error').innerHTML='';
    
    r=ajax.load('<?php echo $base_scripts ?>ajax_ofertas_bd.php?id_oferta='+id_oferta+'&guardar_empresas=1&hcc='+hcc+'&gt='+gt+'&periodo_automatico='+periodo_automatico+ale); 
    if (r.indexOf('Error')!=-1) 
    {   
        $.colorbox.close();
        alert(r);
 ...
 function envia_formulario_borrar_oferta(id_oferta,password,creadas,tipo,tipo_password)
  {
    ...
    var aleatorio='&gg'+(Math.floor(Math.random()*50000))+'=1'
    //var dades=obtenirDadesForm('frm_busqueda');
    dades = 'password='+id_('password_ofe').value; 
    dades+= '&id_oferta='+id_oferta;
...
function envia_formulario_cupon_oferta(string_buscado)
  {
	...
	*/
    ...
    //if (string_buscado.length!=13  && string_buscado.length!=12 && string_buscado.length!=17) 
		//alert(string_buscado.length); 
	//if (string_buscado.length!=11 && string_buscado.length!=12 && string_buscado.length!=13  && string_buscado.length!=17 ) 
