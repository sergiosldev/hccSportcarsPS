...
function request($c)
   {
       ...
       if (isset($_REQUEST[$c]))   
       { 
        if (is_numeric($_REQUEST[$c])) return $_REQUEST[$c];
        else
        return mysqli_real_escape_string($link,strip_tags($_REQUEST[$c]));
       }
       else if (isset($_GET[$c]))
        { 
        if (is_numeric($_GET[$c])) return $_GET[$c];
        else
        return mysqli_real_escape_string($link,strip_tags($_GET[$c]));
        }
       else
        { 
        if (is_numeric($_POST[$c])) return $_POST[$c];
        else
        return mysqli_real_escape_string($link,strip_tags($_POST[$c]));
        }
...
function request_with_tags($c)
   {
	   ...
	   //$save_c = mysqli_real_escape_string($link,$save_c);
	   //$save_c = html_entity_decode($save_c);
	   
	   $tmp=$save_c;
       $save_c = mysqli_real_escape_string($link,$save_c);
	   //IF ($c=='destacados') die('savec6: '.$save_c);
	   //$save_c=pSQL($save_c);
       if ($save_c=='') $save_c=' '; 
...
function field_to_save_insert($c)
{
