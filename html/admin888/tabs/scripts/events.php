<?php  include dirname(__FILE__).'/scripts/config_events.php'; ?>
...
function envia_formulari() {
	...
	if ($('#empresa').val()=="-1") 
	{
		alert("Debe seleccionar la empresa en la que se realiza la reserva");
		return;
	}
    var ale='&gg'+(Math.floor(Math.random()*50000))+'=1'
...
function descarga_mails_dia(){
  window.location= _BASE_URL_ ."/admin888/tabs/scripts/descargar_mails.php?mails_day="+dia_sel+"&ciudad="+ciudad_aux+"&tipus="+tipus_ev
}
function descarga_mails_dia_conf(){
  window.location=_BASE_URL_."/admin888/tabs/scripts/descargar_mails.php?mails_day_conf="+dia_sel+"&ciudad="+ciudad_aux+"&tipus="+tipus_ev
}
function descarga_mails_all(){
  window.location=_BASE_URL_."/admin888/tabs/scripts/descargar_mails.php?mails_all=1"
}

function descarga_mails_barcelona(){
// alert(tipus_ev)   
  window.location=_BASE_URL_."/admin888/tabs/scripts/descargar_mails.php?mails_bcn=1"
}

function descarga_mails_madrid(){
  window.location=_BASE_URL_."/admin888/tabs/scripts/descargar_mails.php?mails_mad=1"
}

function descarga_mails_valencia(){
  window.location=_BASE_URL_."/admin888/tabs/scripts/descargar_mails.php?mails_val=1"
}

function descarga_mails_andalucia(){
  window.location=_BASE_URL_."/admin888/tabs/scripts/descargar_mails.php?mails_and=1"
}
...
function get_graella_instructors() {
  ...
  //alert(r);
  //id_('graella').innerHTML=r;
  id_('form_excel').action='<?php echo $base_scripts ?>graella_instructors.php?data='+dia+'&ciudad='+ciudad_aux+'&tipus='+tipus_ev+'<?php echo $f; ?>'+ale;
  id_('form_excel').submit();
	}
...
<!-- EXCEL -->
<form target="_blank" id="form_excel" action="tabs/download_excel.php" method="POST" >
	<input type="hidden" name = "listado_instructores" id="listado_instructores">
</form>
