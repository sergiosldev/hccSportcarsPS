...
$base_uri = _BASE_URL_;
$base_scripts = _BASE_URL_."/admin888/tabs/scripts/";
...
?> 
        ...
        <div id='form_ofe' style='text-align:left;padding:10px; background:#fff;'>
            ...
            <input type="button" class="boto" name="nueva_oferta" id="nueva_oferta" value ="Nueva Oferta" onclick="nueva_oferta();"><br>  <br>                                
                <div class="tab-row">
                    ...
                    <h4 class="tab" id='tab2'><a href="#" onclick="CambiarTab('',2,2);return(false);">2. Im√°genes.</a> </h4>
        ...
        <!-- Formulasrio para generar el PDF del cup√≥n vista previa -->
...
<script>
    function submit_form_oferta(boton)
    {
	   ...
	   var tab_sel =  1;
		...
		//edita_oferta(id_('id_oferta').value,'',4);
		
		 edita_oferta(id_('idoferta').value,'',4);
		 CambiarTab('opcion',tab_sel,id_('nopciones').value);
		
		
		/*if (id_('nopciones')!=null)
			{
			var numopciones = id_('nopciones').value;
			if (id_('idopcion'+numopciones).value!='')
			id_('boton_vista_previa'+numopciones).style.display='block';
			}
			else id_('boton_vista_previa'+numopciones).style.display='none';
		*/
    }
    ...
    {
        //envia_formulari_oferta(id_('idoferta').value,num_opciones,nlineas_destacados,nlineas_condiciones,nlineas_descripcioncupones);  
    ...
    {
        var ale='&gg'+(Math.floor(Math.random()*50000))+'=1';
		//alert('../scripts/generar_pdf_cupon.php?id_oferta='+id_('idoferta').value+'&id_opcion_oferta='+id_opcion+'&vista_previa=1'+ale);
	...
	function del_tab_opcion(id_opcion,elem)
	{
	   ...
	   var tab_sel =  1;
		...
		<?php
			$texto = utf8_encode('La opciÛn va a ser eliminada. øEst· seguro?');
		?>
        document.body.style.cursor='wait';
	...
	function add_tab_opcion(elem)
	{
		...
		{ 
			alert('<?php echo(utf8_encode('No puede aÒadir una opciÛn porque los datos de la oferta no se han guardado'));?>');
		}
		else if (numopciones>0)
		{
			...
			{	
				if (id_('idopcion'+numopciones).value=='')
				{
				alert('<?php echo(utf8_encode('No puede aÒadir una opciÛn si existe todavÌa alguna con los datos sin guardar'));?>');			
				}
				else 
				{
