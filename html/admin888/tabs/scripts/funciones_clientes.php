...
function GetNumClientes($inOrden,$inDireccion,$inIdOferta=null,$inFiltros=null,$inRegistroInicio=1,$inNRegistrosPagina=100,$inCompraCliente=0,$bajas=true)                                              
{    
    switch ($inOrden)         
    {
        case '0':
    ...
    if (!$bajas)
		$where .= ' and ifnull(eliminado,0)!=1 ';
...
function GetClientes($inOrden,$inDireccion,$inIdOferta=null,$inFiltros=null,$inRegistroInicio=1,$inNRegistrosPagina=100,$inCompraCliente=0,$bajas=true)
{
	...
	//echo(strlen(trim($inFiltros)).'--'.strlen(trim($inFiltros)));
...
//return ($sql);
    ...
    //while ($row = mysql_fetch_assoc($query))
    {
        ...
        $Usuario = new UsuarioHCC(pSQL($row['id_usuario']), pSQL($row['email']),pSQL($row['password']),pSQL($row['nombre']),pSQL($row['apellidos']),pSQL($row['fecha_nacimiento']), 
