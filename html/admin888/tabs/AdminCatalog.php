...
class AdminCatalog extends AdminTab
{
	...
	public function display()
	{
		...
		if (!Tools::getValue('id_product'))
		{
			/* mts 13012013, editor para modificar el texto de la pantalla de inicio */
			echo('<a id="boton_texto_inicio" style="text-decoration:underline;" href="javascript:mostrar_ocultar_texto_inicio();" ><h2>Pantalla de Inicio</h2></a><br>');
			echo('<script> ');
			echo('function mostrar_ocultar_texto_inicio()
				  {');
			echo(' if (document.getElementById(\'boton_texto_inicio\').innerHTML==\'<h2>Pantalla de Inicio</h2>\') 
				   {');
			echo('    mostrar_inicio();document.getElementById(\'boton_texto_inicio\').innerHTML = \'<h2>Cerrar Ventana Texto Inicio</h2>\';');
			echo(' } 
				   else 
				   {
					  document.getElementById(\'boton_texto_inicio\').innerHTML = \'<h2>Pantalla de Inicio</h2>\';ocultar_inicio();
				   }
				  }');
			echo('</script>' );
			include_once(PS_ADMIN_DIR.'/tabs/inicio.php');
		}
		//font-weight:bolc;color:#268CCD;font-size:1.4em;
		/* fin modif. mts. */
