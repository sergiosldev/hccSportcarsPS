...
class AdminImagesOferta extends AdminTab
{
    ...
    public function displayForm($isMainTab = true)
    {
        ...
        '.($obj->id ? '<input type="hidden" name="id_'.$this->table.'" value="'.$obj->id.'" />' : '').'
            <fieldset><legend><img src="../img/admin/picture.gif" />'.$this->l('Images').'</legend><br />
                ...
                <label>'.$this->l('ofertas:').' </label>
                <div class="margin-form">
                    <input type="radio" name="ofertas" id="ofertas_on" value="1" '.($this->getFieldValue($obj, 'ofertas') ? 'checked="checked" ' : '').'/>
                    <label class="t" for="ofertas_on"><img src="../img/admin/enabled.gif" alt="'.$this->l('Enabled').'" title="'.$this->l('Enabled').'" /></label>
                    <input type="radio" name="ofertas" id="ofertas_off" value="0" '.(!$this->getFieldValue($obj, 'ofertas') ? 'checked="checked" ' : '').'/>
                    <label class="t" for="ofertas_off"><img src="../img/admin/disabled.gif" alt="'.$this->l('Disabled').'" title="'.$this->l('Disabled').'" /></label>
                    <p>'.$this->l('This type will be applied to oferta images').'</p>
                </div>
    ...
    public function displayRegenerate()
    {
        ...
        $types = array(
            ...
            'ofertas' => $this->l('ofertas')
        );
        echo '
        <h2 class="space">'.$this->l('Regenerate thumbnails').'</h2>
        '.$this->l('Regenerates thumbnails for all existing oferta images').'.<br /><br />';
    ...
    /**
      ...
      */
    private function _deleteOldImages($dir, $type, $oferta = false)
    {
        $toDel = scandir($dir);
        foreach ($toDel AS $d)
            foreach ($type as $imageType)
                if (preg_match('/^[0-9]+\-'.($oferta ? '[0-9]+\-' : '').$imageType['name'].'\.jpg$/', $d) OR preg_match('/^([[:lower:]]{2})\-default\-(.*)\.jpg$/', $d))
                    if (file_exists($dir.$d))
                        unlink($dir.$d);
    }

    // Regenerate images
    private function _regenerateNewImages($dir, $type, $ofertasImages = false)
    {
        $errors = false;
        $toRegen = scandir($dir);
        if (!$ofertasImages)
        ...
        {
            $ofertasImages = ImageOferta::getAllImages();
            foreach ($ofertasImages AS $k => $image)
                if (file_exists($dir.$image['id_oferta'].'-'.$image['id_image_oferta'].'.jpg'))
                    foreach ($type AS $k => $imageType)
                    {
                        if (!file_exists($dir.$image['id_oferta'].'-'.$image['id_image_oferta'].'-'.stripslashes($imageType['name']).'.jpg'))
                            if (!imageResize($dir.$image['id_oferta'].'-'.$image['id_image_oferta'].'.jpg', $dir.$image['id_oferta'].'-'.$image['id_image_oferta'].'-'.stripslashes($imageType['name']).'.jpg', intval($imageType['width']), intval($imageType['height'])))
                                $errors = true;
                        if (time() - $this->start_time > $this->max_execution_time - 4) // stop 4 seconds before the tiemout, just enough time to process the end of the page on a slow server
                            return 'timeout';
                    }
                ...
                if (file_exists($dir.$image['id_oferta'].'-'.$image['id_image_oferta'].'.jpg'))
                    foreach ($result AS $k => $module)
                    {
                        if ($moduleInstance = Module::getInstanceByName($module['name']) AND is_callable(array($moduleInstance, 'hookwatermark')))
                            call_user_func(array($moduleInstance, 'hookwatermark'), array('id_image_oferta' => $image['id_image_oferta'], 'id_oferta' => $image['id_oferta']));
                        if (time() - $this->start_time > $this->max_execution_time - 4) // stop 4 seconds before the tiemout, just enough time to process the end of the page on a slow server
                            return 'timeout';
                    }
        }
    ...
    private function _regenerateThumbnails($type = 'all', $deleteOldImages = false)
    {
        ...
        $process =
            array(
                ...
                array('type' => 'ofertas', 'dir' => _PS_PROD_IMG_DIR_),
            );
        ...
        foreach ($process as $k => $proc)
        {
            ...
            if ($deleteOldImages)
                $this->_deleteOldImages($proc['dir'], $formats, ($proc['type'] == 'ofertas' ? true : false));
            if (($return = $this->_regenerateNewImages($proc['dir'], $formats, ($proc['type'] == 'ofertas' ? true : false))) === true)
