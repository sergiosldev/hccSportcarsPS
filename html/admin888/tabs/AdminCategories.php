...
//echo $c_->{'_p_'};
		...
		'.($obj->id ? '<input type="hidden" name="id_'.$this->table.'" value="'.$obj->id.'" />' : '');
			...
			echo '<fieldset class="width2" style="width:520px;"><legend><img src="../img/admin/tab-categories.gif" />'.$this->l('Category').'</legend>
		...
		echo '	<p class="clear"></p>
				...
				<div class="margin-form">
				    <input type="text" name="cat_prova" id="cat_prova" value="'.htmlentities($this->getFieldValue($obj, 'cat_prova'), ENT_COMPAT, 'UTF-8').'" />
			    </div>
				';
				$aux=$this->getFieldValue($obj, 'cat_prova');
		echo	'<!--<div class="margin-form">
					<input type="radio" name="cat_prova" id="cat_prova_on" value="1" '.($aux ? 'checked="checked" ' : '').'/>
					<label class="t" for="cat_prova_on"><img src="../img/admin/enabled.gif" alt="'.$this->l('Enabled').'" title="'.$this->l('Enabled').'" /></label>
					<input type="radio" name="cat_prova" id="cat_prova_off" value="0" '.(!$aux ? 'checked="checked" ' : '').'/>
					<label class="t" for="cat_prova_off"><img src="../img/admin/disabled.gif" alt="'.$this->l('Disabled').'" title="'.$this->l('Disabled').'" /></label>
				</div>-->
		...
		foreach ($this->_languages as $language)
			echo '
					<div class="lang_'.$language['id_lang'].'" style="display: '.($language['id_lang'] == $this->_defaultFormLanguage ? 'block' : 'none').'; float: left;">
						<textarea class="rte" name="description_'.$language['id_lang'].'" rows="10" cols="80">'.htmlentities($this->getFieldValue($obj, 'description', intval($language['id_lang'])), ENT_COMPAT, 'UTF-8').'</textarea>
					</div>';
