...
class AdminPayment extends AdminTab
{
	...
	public function postProcess()
	{
		...
		elseif (Tools::isSubmit('submitModulecountry'))
		...
		foreach ($this->paymentModules as $module)
			{
				...
				echo '
					...
					onclick="javascript:document.location.href=\'index.php?tab=AdminModules&install='.urlencode($module->name).'&token='.$tokenModules.'&bpay\'" />'
					: '<input type="button" class="button small" name="Uninstall" value="'.$this->l('Uninstall').'"
					onclick="'.(empty($module->confirmUninstall) ? '' : 'if(confirm(\''.addslashes($module->confirmUninstall).'\')) ').'document.location.href=\'index.php?tab=AdminModules&uninstall='.urlencode($module->name).'&token='.$tokenModules.'&bpay\';" />').'</td>
				</tr>';
			}
		echo '</table>';
	}
