...
class AdminCMS extends AdminTab
{
  ...
  <div class="margin-form">
		<input type="text" name="tipus" id="tipus" value="'.htmlentities($this->getFieldValue($obj, 'tipus'), ENT_COMPAT, 'UTF-8').'" />
  </div>';
		...
		foreach ($this->_languages as $language)
			echo '	<div id="link_rewrite_'.$language['id_lang'].'" style="display: '.($language['id_lang'] == $this->_defaultFormLanguage ? 'block' : 'none').'; float: left;">
						<input size="30" type="text" name="link_rewrite_'.$language['id_lang'].'" onchange="this.value = str2url(this.value); updateFriendlyURL();" value="'.htmlentities($this->getFieldValue($obj, 'link_rewrite', intval($language['id_lang'])), ENT_COMPAT, 'UTF-8').'" /><sup> *</sup>
					</div>';
