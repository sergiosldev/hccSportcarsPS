...
define('_PS_OFER_IMG_DIR_',         _PS_IMG_DIR_.'o/');
...
function select_temas($path)
    {
    $dir=opendir($path);	
	$f='
	<form action="" id="tema" >
	<select  name="theme" onchange="document.getElementById(\'tema\').submit()">';	
	 $f.='<option selected>'.$_REQUEST['theme'].'</option>'; 
	while($ele_dir=readdir($dir))
        {
        if(is_dir($path.'/'.$ele_dir) && !in_array($ele_dir,array('.','..')))// si es directori
           { $f.='<option>'.$ele_dir.'</option>'; }   
        }
     return $f.'
	 </select>
	 </form>
...
// echo select_temas(_PS_ROOT_DIR_.'/themes');
