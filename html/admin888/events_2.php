...
include dirname(__FILE__).'/scripts/config_events_2.php'; ?>
...
function reubicar(id,hora) //quan esborres pilot
{
  var ale='&gg'+(Math.floor(Math.random()*50000))+'=1'
  id_('msg_error').innerHTML='';
  document.getElementById('form_reubicar').reset()
  $.colorbox({width:"60%", inline:true, href:"#form_reubicar",open:true});
  //id_('alta').style.display="block"
  r=ajax.load('<?php echo $base_scripts ?>ajax_2.php?id_reubica='+id+ale+'&ciudad='+ciudad_aux+'&hora='+hora);
  eval(r);
  id_('edicio').value='true';
}
